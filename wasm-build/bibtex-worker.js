/*
 * bibtex-worker.js — authored Web Worker controller for BibTeX WASM
 */
"use strict";
...
function writeTexmfCnf() {
  var cnf = [
    ...
    // Don't wrap .bbl output. The compiled-in default wraps around column 60,
    // which breaks long "%%%" comment banners in .bst files (e.g. acmart's
    // ACM-Reference-Format) across lines — the continuation loses its "%" and
    // becomes typeset garbage inside thebibliography, triggering "Improper
    // \\prevdepth" -> "missing \\item" -> "TeX capacity exceeded". 1000 is far
    // above any real .bst banner (~79 cols) yet safely below BibTeX's buf_size
    // (default 20000): max_print_line >= buf_size trips BibTeX's init sanity
    // check ("...is a bad...") and aborts before writing any .bbl.
    "max_print_line = 1000",
    ""
  ].join("\n");
...
importScripts("wasmtex-bibtex.js");
