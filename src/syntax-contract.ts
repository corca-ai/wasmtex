...
export const LATEX_SYNTAX_SCHEMA_VERSION = 4 as const
...
export type LatexNotationNodeKind =
  | 'token'
  | 'sequence'
  | 'group'
  | 'command'
...
export function assertLatexSyntaxSchemaVersion(value: unknown): asserts value is {
  ...
  if (
    ...
    value.schemaVersion !== LATEX_SYNTAX_SCHEMA_VERSION
  ) {
    const actual =
      typeof value === 'object' && value !== null && 'schemaVersion' in value
        ...
        : 'missing'
    throw new Error(
      `Unsupported LaTeX syntax schema ${actual}; expected ${LATEX_SYNTAX_SCHEMA_VERSION}`,
    )
  }
}
