...
export async function persistIfNeeded(
  ...
  if (state.downloadCount === state.lastPersisted) return
  const target = state.downloadCount // capture now; commit only on success
  state.inFlight = true
  try {
    await save()
    state.lastPersisted = target
  } catch {
    // Best-effort: ignore quota / IndexedDB failures; the watermark stays behind so a
    // later compile retries the save instead of treating these files as persisted.
  } finally {
