...
| `texliveDependencies` | `TexliveDependencySet` | The exact TeX Live dependency set of the compile — the union across rerun passes of every resource the TeX passes resolved (`files`, each with the kpathsea request `filename` and, when it differs, the mirror `candidate`) or found absent (`notFound`). Names only, bound to `texliveVersion` and `profile`; `complete` is false when a retention bound dropped entries. Replay it through [`warmup({ dependencies })`](warmup.md#exact-dependency-prefetch-dependencies) next session. |
