...
**Prebuilt format (cold-start fast path).** Building the format from scratch on
every cold start dominated the first-compile time (LuaLaTeX ~13 s; XeLaTeX ~10–13 s,
its format build also pulling ~245 kernel files over sync XHR). Instead, CI extracts
the format once (`scripts/extract-luatex-format.mjs` / `extract-xetex-format.mjs` run
the freshly-built engine's `-ini` and capture the bytes) and ships it next to the
engine. At init the engine fetches it **in parallel with the worker boot** into
`fmtBytes`, so the first compile re-injects the format and typesets directly — no
`compileformat`. Measured (same CDN edge): **XeLaTeX cold first PDF ~10.5 s → ~1.9 s
(−82%)**, CDN fetches 386 → 138.
...
**Bloom filter + built-in warmup.** After the format build is gone, the remaining
cold-start cost is the worker fetching its runtime (luaotfload/lualibs `.lua`,
fonts) synchronously, one file at a time, plus wasted lookups for files that don't
exist. Two fixes (mirroring pdfTeX): the worker loads the CDN **bloom filter**
(`loadbloom`) and skips the sync XHR for any file it says is absent; and the engine
**prefetches** the known first-compile file set in parallel (overlapping worker
boot) and injects it via `preloadtexlive`, so the worker finds everything locally
and never blocks. The file set is `src/engine/luatex-manifest.ts`, generated by
`scripts/gen-luatex-manifest.mjs` from an instrumented compile (regenerate when the
runtime changes). The sends are fire-and-forget, so a stale worker (without these
commands) simply ignores them and fetches on demand. Combined with the prebuilt
format, this took a local cold first compile from ~13 s to ~2.6 s (time to first
PDF), with wasted 4xx lookups dropping from ~116 to ~1.
...
**Recompile (edit) latency.** Within a session, body edits recompile in ~0.5 s
(luaotfload doc) / ~0.4 s (no fontspec): the prebuilt format is injected into the
work dir only once (it persists in MEMFS across recompiles — re-writing the multi-MB
`.fmt` every edit was pure overhead), and the format build is already skipped. The
remaining cost is genuine preamble re-execution (notably luaotfload's reload for
fontspec docs, ~130 ms) plus typesetting. A full **preamble snapshot** (the pdfTeX
technique — precompile the preamble into a sub-format so body edits skip it) is the
next lever, but for LuaLaTeX it is deferred: luaotfload's Lua state is hostile to
`\dump`-based precompilation, so it needs a careful worker port with graceful
fallback, for a moderate gain on top of an already-interactive recompile.
...
**Validated**: builds cleanly and passes the by-name *and* Korean `xeCJK` tests
(by name and by filename) against the live CDN. This is how the `xetexfontlist.txt`
parser bug was diagnosed (printf tracing of the font manager).
...
LuaLaTeX is wired end-to-end on the app side (detection → `WasmTexLuatexEngine`
→ own glue → graceful fallback), its TeX Live assets are mirrored — the `.lua`
runtime (luaotfload, lualibs) lives under format 51 — and the engine itself is built
from source.
...
>
> The DB is generated against **exactly** the mirror's fonts (`--fonts-dir`, issue #73):
...
### Font-by-name (`xetexfontlist.txt`) — WORKS
...
>    record and silently loads nothing — which is exactly why by-name appeared
>    broken. The generator emits the 5th field to match.
...
> against the live CDN (diagnosed with a from-source instrumented build on a strong
> Linux host; the fix is in the *data*, so it works with the prebuilt engine too).
...
We investigated an emscripten SIMD (`-msimd128`) build of pdfTeX for faster
compiles. Findings and decision:

- TeX's algorithm is **inherently sequential**, so SIMD only accelerates a few
  vectorizable inner loops — the realistic upside for a typical compile is small.
  Threads (pthreads) would additionally require cross-origin isolation
  (COOP/COEP) and mostly help IO/libraries, not TeX itself.
- A SIMD build is a **separate artifact** that must be gated by a runtime
  capability check (older browsers lack SIMD) with a scalar fallback.

**Decision:** ship the runtime capability detector now
(`wasmSimdSupported()` in `src/engine/wasm-features.ts`) and **defer building the
SIMD pdfTeX artifact** until a measured `wasm-build.yml` experiment quantifies the
compile-time gain against the artifact-size cost — that empirical benchmark
(compile time, artifact size, browser support) requires the emscripten build
pipeline, not pure-TS CI. When the SIMD artifact is built, it must be served only
when `wasmSimdSupported()` returns `true`, falling back to the scalar build
otherwise.
