...
import { IncrementalCompiler as ue } from "./engine/incremental.js";
import { RerunController as A, signatureOf as j } from "./engine/rerun-controller.js";
import { syncAllFilesToEngine as M } from "./fs/engine-sync.js";
import { VirtualFS as N } from "./fs/virtual-fs.js";
import { parseAuxFile as P } from "./lsp/aux-parser.js";
import { parseBibFile as F, rebuildBibIndex as I } from "./lsp/bib-parser.js";
import { ProjectIndex as L } from "./lsp/project-index.js";
import { parseTraceFile as R } from "./lsp/trace-parser.js";
import { MakeindexEngine as z } from "./engine/makeindex-engine.js";
import { buildTexliveDependencySet as B, mergeTexliveDependencySets as V } from "./engine/texlive-dependencies.js";
import { PREAMBLE_SNAPSHOT_JOBNAME as H, defaultFigureWorkers as U, detectAutoBlocker as de, detectTikzExternalization as fe, figureJobSource as pe, mainJobSource as me, parseFigureList as he, parseFigureMd5 as ge } from "./engine/tikz-externalization.js";
...
async function _e(e, t = {}) {
	let n = e.mainFile ?? "main.tex", r = e.files?.[n], i = typeof r == "string" ? r : "", { injectDocumentMetadata: a, documentClassOf: o, CLASS_SUPPORT: s } = await q(), c = a(i, t), l = o(i), u = l && s[l] || "unknown", d = J(l, u, c.injected), f = new $({
		...e,
		files: {
			...e.files,
			[n]: c.source
	...
	try {
		return await f.init(), await K(await f.compile(), c, l, u, d);
	} finally {
		f.dispose();
	}
}
async function K(e, t, n, r, i) {
	let { kernelLacksTagging: a, inspectPdfTagging: o } = await q(), s = !a(e.log);
...
var Se = [
	...
	constructor(e = {}) {
		this.opts = e, this.mainFile = e.mainFile ?? "main.tex", this.assetBaseUrl = xe(e.assetBaseUrl), this.fs = new N({ empty: !0 });
	...
	engineBaseOpts() {
		let e = {
			...
			heapCheckpoints: !!this.opts.incremental && !ve(),
	...
	async ensureEngine() {
		if (this.detection = le(this.mainSource(), this.opts.engine), !(this.engine && this.detection.engine === this.engineKind)) {
			if (this.engine?.terminate(), this.engineKind = this.detection.engine, this.engine = se(this.detection.engine, this.engineBaseOpts()), this.attachLoadProgress(this.engine), this.opts.onEngineSelected) {
			...
			this.incremental = this.opts.incremental && this.engine instanceof k ? new ue(this.engine, { mainFile: this.mainFile }) : null, this.heap = this.opts.incremental && this.engine instanceof k ? new (await (import("./engine/heap-checkpoints.js"))).HeapCheckpointCompiler(this.engine, { mainFile: this.mainFile }) : null;
			try {
	...
	async compileIdle() {
		...
		if (n) return n;
		let r = await this.tryHeapResume(t) ?? await e.compile(this.heapArms()), i = new Set(r.pdfConversionInputs ?? []), a = [r.telemetry?.resolver];
		r = await this.applyTikzExternalization(r, t, a), Z(i, r);
		let o = r.telemetry?.tikzExternalization, s = await this.runAuxStages(r), c = new A();
		for (; (r.success || r.pdf) && !(!c.decide(r.log, j(r.semanticTrace ?? r.log)).rerun && !s);) await this.syncModifiedFilesToEngine(), r = await e.compile(this.heapArms()), a.push(r.telemetry?.resolver), Z(i, r), s = await this.runAuxStages(r);
		return this.heap?.noteFull(this.mainSource(), this.projectTexFiles(), r), o && (r.telemetry ??= { diagnostics: E(r.log) }, r.telemetry.tikzExternalization = o), ye(r, i), this.attachTexliveDependencies(r, a), await this.updateMetadata(r), this.attachDependencyManifest(r), await this.attachCompletionSnapshot(r), this.incremental?.noteFull(this.mainSource(), this.projectTexFiles(), r.synctex), r;
	}
	...
	async loadProject(e) {
		this.fs = new N({ empty: !0 }), this.projectIndex = new L(), this.generatedFiles.clear(), this.generatedDependencyObservations.clear(), this.currentAuxiliaryDependencies.clear(), this.lastFullDependencyManifest = void 0, this.incremental?.reset();
	...
	attachTexliveDependencies(e, t) {
		...
		for (let e of Se) n.add(`${r}.${e}`);
		let i = B(this.opts.texliveVersion ?? "2025", this.completionProfile(), t, { excludeNames: n });
		i && (this.sessionDependencies = V(this.sessionDependencies, i), e.telemetry ??= { diagnostics: E(e.log) }, e.telemetry.texliveDependencies = this.sessionDependencies);
	}
	...
	async syncAllFilesToEngine() {
		let e = this.engine;
		!e || this.unavailable || await M(this.fs, {
	...
	tikzExternalizationKind(e = this.mainSource()) {
		let t = fe(e, this.opts.tikzExternalization?.mode ?? "document");
		return t === "inject" ? this.tikzAutoDisabled ? null : (this.tikzAutoBlocker = de(e, this.projectTexFiles().values()), this.tikzAutoBlocker ? null : t) : t;
	}
	...
	async externalizeTikzFigures(e, t, n) {
		...
		return l !== e && n.push(l.telemetry?.resolver), o.pictureErrors = be(l, s), c && (l.log += c), X(l, o);
	}
	async runTikzFigureJobs(e, t, n) {
		...
		let c = globalThis.navigator, l = this.opts.tikzExternalization?.workers ?? U(c?.hardwareConcurrency, c?.deviceMemory), u = await this.ensureTikzPool(l);
		u.retain(s);
	...
	async readTikzFigureList(e, t) {
		...
		let r = t ? [H, e] : [e, H];
		for (let e of r) {
			let t = he(await n.readFile(`${e}.figlist`));
	...
	*projectFileEntries() {
	...
	async exportAccessiblePdf(e = {}) {
		this.ensureInitialized();
		let t = this.mainSource(), { injectDocumentMetadata: n, documentClassOf: r, CLASS_SUPPORT: i } = await q(), a = n(t, e), o = r(t), s = o && i[o] || "unknown", c = J(o, s, a.injected);
		return K(await this.compileForExport(a.source), a, o, s, c);
	}
	...
	updateBibIndex() {
		I(this.fs, this.projectIndex);
	}
	async updateMetadata(e) {
		if (!this.engine) return;
		let t = this.mainFile.replace(/\.tex$/, ""), n = await this.engine.readFile(`${t}.aux`);
		if (n && this.projectIndex.updateAuxData(P(n)), e.engineCommands?.length && this.projectIndex.updateEngineCommands(e.engineCommands), e.semanticTrace && this.projectIndex.updateSemanticTrace(R(e.semanticTrace)), e.inputFiles?.length) for (let t of e.inputFiles) {
			let e = O(t);
			if (!e) continue;
	...
	runClientBiblatexLite(e, t, n) {
		let r = Object.entries(n).flatMap(([e, t]) => F(t, e)), i = m(t), a = i.includes("*") ? r.map((e) => e.key) : i;
		return p({
			entries: r,
			citedKeys: a,
			sort: d(e)
	...
	async ensureMakeindexEngine() {
		...
		let e = new z(this.auxEngineOpts());
