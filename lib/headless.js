...
import { createXindyBackend as C } from "./engine/xindy-backend.js";
import { COMPLETION_SNAPSHOT_MAX_ESTIMATED_BYTES as w, COMPLETION_SNAPSHOT_SCHEMA_VERSION as T, CompletionFileDigestCache as E, createCompletionSnapshot as ne } from "./engine/completion-snapshot.js";
import { BibtexEngine as re } from "./engine/bibtex-engine.js";
import { buildDiagnostics as D, parseTexErrors as O } from "./engine/parse-errors.js";
import { buildDependencyManifest as ie, buildIncrementalDependencyManifest as k, normalizeProjectDependencyPath as A } from "./engine/dependency-manifest.js";
...
import { MakeindexEngine as ae } from "./engine/makeindex-engine.js";
import { buildTexliveDependencySet as G, mergeTexliveDependencySets as K } from "./engine/texlive-dependencies.js";
import { PREAMBLE_SNAPSHOT_JOBNAME as q, TikzFigurePool as J, defaultFigureWorkers as oe, detectAutoBlocker as se, detectTikzExternalization as ce, figureJobSource as le, mainJobSource as ue, parseFigureList as Y, parseFigureMd5 as de } from "./engine/tikz-externalization.js";
...
function Z(e) {
	return O(e).filter((e) => !/Reference .* undefined|Citation .* undefined|There were undefined (?:references|citations)|Label\(s\) may have changed|Rerun to get/i.test(e.message));
}
function Q(e, t) {
	return e.telemetry ??= { diagnostics: D(e.log) }, e.telemetry.tikzExternalization = t, e;
}
function fe(e, t) {
...
var me = [
	...
	constructor(e = {}) {
		this.opts = e, this.mainFile = e.mainFile ?? "main.tex", this.assetBaseUrl = pe(e.assetBaseUrl), this.fs = new z({ empty: !0 });
	...
	async compileIdle() {
		...
		return a && (r.telemetry ??= { diagnostics: D(r.log) }, r.telemetry.tikzExternalization = a), this.attachTexliveDependencies(r, i), await this.updateMetadata(r), this.attachDependencyManifest(r), await this.attachCompletionSnapshot(r), this.incremental?.noteFull(this.mainSource(), this.projectTexFiles(), r.synctex), r;
	}
	...
	toCompileResult(e, t) {
		let n = {
			...
			telemetry: { diagnostics: D(e.log) }
		};
		return n.telemetry.dependencyManifest = k(this.mainFile, this.lastFullDependencyManifest), n;
	}
	setFile(e, t) {
	...
	attachTexliveDependencies(e, t) {
		...
		for (let e of me) n.add(`${r}.${e}`);
		let i = G(this.opts.texliveVersion ?? "2025", this.completionProfile(), t, { excludeNames: n });
		i && (this.sessionDependencies = K(this.sessionDependencies, i), e.telemetry ??= { diagnostics: D(e.log) }, e.telemetry.texliveDependencies = this.sessionDependencies);
	}
	attachDependencyManifest(e) {
		e.telemetry ??= { diagnostics: D(e.log) };
		let t = ie({
	...
	async attachCompletionSnapshot(e) {
		...
		}))), i = t.getCompletionObservation?.(), a = await ne({
			engine: this.engineKind,
			root: n,
		...
		n !== this.mainFile || t !== this.engine || this.fs.getModifiedFiles().length > 0 || (e.telemetry ??= { diagnostics: D(e.log) }, e.telemetry.completionSnapshot = a, this.projectIndex.updateCompletionSnapshot(a));
	}
	...
	tikzExternalizationKind(e = this.mainSource()) {
		let t = ce(e, this.opts.tikzExternalization?.mode ?? "document");
		return t === "inject" ? this.tikzAutoDisabled ? null : (this.tikzAutoBlocker = se(e, this.projectTexFiles().values()), this.tikzAutoBlocker ? null : t) : t;
	}
	...
	async applyTikzExternalization(e, t, n) {
		return t && (e.success || e.pdf) ? this.externalizeTikzFigures(e, t, n) : (this.opts.tikzExternalization?.mode === "auto" && this.tikzAutoBlocker && (e.telemetry ??= { diagnostics: D(e.log) }, e.telemetry.tikzExternalization = {
			...$("auto"),
	...
	async externalizeTikzFigures(e, t, n) {
		...
		if (t === "inject" && (a.inline || o.failed.length > 0)) {
			...
			return n.push(e.telemetry?.resolver), Q(e, o);
		}
		let l = o.compiled > 0 ? await r.compile() : e;
		return l !== e && n.push(l.telemetry?.resolver), o.pictureErrors = fe(l, s), c && (l.log += c), Q(l, o);
	}
	async runTikzFigureJobs(e, t, n) {
		...
		if (t === "inject" && s.length < 3) {
			...
			return t.blocked = "too-few-pictures", e.telemetry ??= { diagnostics: D(e.log) }, e.telemetry.tikzExternalization = t, {
				telemetry: t,
				errors: [],
				failureLog: "",
				inline: !0
		...
		let c = globalThis.navigator, l = this.opts.tikzExternalization?.workers ?? oe(c?.hardwareConcurrency, c?.deviceMemory);
		this.tikzPool ??= new J(() => this.spawnFigureCompiler(), l, this.mainFile);
		let u = this.tikzPool;
		u.retain(s);
		let d = await Promise.all(s.map((e) => r.readFile(`${e}.md5`))), f = s.map((e, t) => ({
			...
			...$(t === "inject" ? "auto" : "document"),
			figures: s.length,
			reused: s.length - f.length,
			workers: Math.min(l, Math.max(1, f.length))
		};
		if (e.telemetry ??= { diagnostics: D(e.log) }, e.telemetry.tikzExternalization = p, f.length === 0) return {
			telemetry: p,
			errors: X(u, s),
			failureLog: ""
		};
	...
	async readTikzFigureList(e, t) {
		...
		let r = t ? [q, e] : [e, q];
		for (let e of r) {
			let t = Y(await n.readFile(`${e}.figlist`));
	...
	*projectFileEntries() {
	...
	async ensureBibtexEngine() {
		...
		let e = new re(this.auxEngineOpts());
	...
	async ensureMakeindexEngine() {
		...
		let e = new ae(this.auxEngineOpts());
...
//#endregion
export { e as BIBER_STAGE, c as BIBLIOGRAPHY_STAGE, t as BIBTEX_STAGE, n as BackendRegistry, w as COMPLETION_SNAPSHOT_MAX_ESTIMATED_BYTES, T as COMPLETION_SNAPSHOT_SCHEMA_VERSION, r as INDEX_STAGE, v as MemoryCacheStore, he as WasmTexCompiler, y as backendCacheKey, l as biblatexLiteBackend, b as contentKey, o as createBiberBackend, i as createJsonTextBackend, te as createMakeindexBackend, a as createRemoteBackend, C as createXindyBackend, u as detectBiblatexBackend, d as detectBiblatexSort, f as detectBibliographyMode, x as detectIndexUse, p as generateBiblatexBbl, m as parseBcfCitedKeys, s as runRemoteBiber, g as runRemoteBibliography, S as runRemoteIndex, _ as selectBiblatexBackend, ee as withCache };
