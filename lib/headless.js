...
import { buildTexliveDependencySet as Z } from "./engine/texlive-dependencies.js";
...
var $ = [
	...
	constructor(e = {}) {
		this.opts = e, this.mainFile = e.mainFile ?? "main.tex", this.assetBaseUrl = Q(e.assetBaseUrl), this.fs = new W({ empty: !0 });
	...
	async compileIdle() {
		...
		let t = await e.compile(), n = [t.telemetry?.resolver], r = await this.runAuxStages(t), i = new V();
		for (; (t.success || t.pdf) && !(!i.decide(t.log, H(t.semanticTrace ?? t.log)).rerun && !r);) await this.syncModifiedFilesToEngine(), t = await e.compile(), n.push(t.telemetry?.resolver), r = await this.runAuxStages(t);
	...
	attachTexliveDependencies(e, t) {
		...
		let i = Z(this.opts.texliveVersion ?? "2025", this.completionProfile(), t, { excludeNames: n });
		i && (e.telemetry ??= { diagnostics: j(e.log) }, e.telemetry.texliveDependencies = i);
	}
