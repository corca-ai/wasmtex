...
import { IncrementalCompiler as C } from "./engine/incremental.js";
import { detectIndexUse as c, runRemoteIndex as I } from "./engine/index-backend.js";
import { INDEX_STAGE as ue, createMakeindexBackend as de } from "./engine/index-backend.js";
import { MakeindexEngine as $ } from "./engine/makeindex-engine.js";
import { buildDiagnostics as M, parseTexErrors as S } from "./engine/parse-errors.js";
import { RerunController as T, signatureOf as j } from "./engine/rerun-controller.js";
...
import { parseAuxFile as R } from "./lsp/aux-parser.js";
import { rebuildBibIndex as A, parseBibFile as U } from "./lsp/bib-parser.js";
import { ProjectIndex as f } from "./lsp/project-index.js";
import { parseTraceFile as W } from "./lsp/trace-parser.js";
import { BackendRegistry as pe, createJsonTextBackend as be, createRemoteBackend as ge } from "./engine/backend-registry.js";
import { MemoryCacheStore as Fe, contentKey as we, withCache as ye } from "./engine/content-cache.js";
import { createXindyBackend as Ee } from "./engine/xindy-backend.js";
function z(l) {
...
class ne {
  ...
  async ensureEngine() {
    if (this.detection = k(this.mainSource(), this.opts.engine), !(this.engine && this.detection.engine === this.engineKind)) {
      this.engine?.terminate(), this.engineKind = this.detection.engine, this.engine = B(this.detection.engine, this.engineBaseOpts()), this.incremental = this.opts.incremental && this.engine instanceof v ? new C(this.engine, { mainFile: this.mainFile }) : null;
      try {
  ...
  async compile() {
    ...
    return await this.updateMetadata(i), this.incremental?.noteFull(this.mainSource(), this.projectTexFiles()), i;
  }
  ...
  async updateMetadata(e) {
    ...
    if (t && this.projectIndex.updateAuxData(R(t)), e.engineCommands?.length && this.projectIndex.updateEngineCommands(e.engineCommands), e.semanticTrace && this.projectIndex.updateSemanticTrace(W(e.semanticTrace)), e.inputFiles?.length)
  ...
  async maybeRunMakeindex(e) {
    ...
    if (!n?.trim()) return !1;
    const s = { idx: n }, a = await I(this.opts.backends, s) ?? await this.runClientMakeindex(t, n);
...
export {
  ...
  pe as BackendRegistry,
  ue as INDEX_STAGE,
  Fe as MemoryCacheStore,
  ne as WasmTexCompiler,
  ce as biblatexLiteBackend,
  we as contentKey,
  re as createBiberBackend,
  be as createJsonTextBackend,
  de as createMakeindexBackend,
  ge as createRemoteBackend,
  Ee as createXindyBackend,
