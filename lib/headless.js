...
import { resolveEngine as ee } from "./engine/engine-select.js";
...
import { buildTexliveDependencySet as X, mergeTexliveDependencySets as Z } from "./engine/texlive-dependencies.js";
...
var $ = [
	...
	sessionDependencies;
	constructor(e = {}) {
		this.opts = e, this.mainFile = e.mainFile ?? "main.tex", this.assetBaseUrl = Q(e.assetBaseUrl), this.fs = new U({ empty: !0 });
	...
	async ensureEngine() {
		if (this.detection = ee(this.mainSource(), this.opts.engine), !(this.engine && this.detection.engine === this.engineKind)) {
			this.engine?.terminate(), this.engineKind = this.detection.engine, this.engine = L(this.detection.engine, this.engineBaseOpts()), this.incremental = this.opts.incremental && this.engine instanceof I ? new z(this.engine, { mainFile: this.mainFile }) : null;
			try {
	...
	async compileIdle() {
		...
		let t = await e.compile(), n = [t.telemetry?.resolver], r = await this.runAuxStages(t), i = new B();
		for (; (t.success || t.pdf) && !(!i.decide(t.log, V(t.semanticTrace ?? t.log)).rerun && !r);) await this.syncModifiedFilesToEngine(), t = await e.compile(), n.push(t.telemetry?.resolver), r = await this.runAuxStages(t);
		return this.attachTexliveDependencies(t, n), await this.updateMetadata(t), this.attachDependencyManifest(t), await this.attachCompletionSnapshot(t), this.incremental?.noteFull(this.mainSource(), this.projectTexFiles(), t.synctex), t;
	}
	...
	attachTexliveDependencies(e, t) {
		...
		for (let e of $) n.add(`${r}.${e}`);
		let i = X(this.opts.texliveVersion ?? "2025", this.completionProfile(), t, { excludeNames: n });
		i && (this.sessionDependencies = Z(this.sessionDependencies, i), e.telemetry ??= { diagnostics: j(e.log) }, e.telemetry.texliveDependencies = this.sessionDependencies);
	}
