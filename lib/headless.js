...
import { COMPLETION_SNAPSHOT_MAX_ESTIMATED_BYTES as E, COMPLETION_SNAPSHOT_SCHEMA_VERSION as D, createCompletionSnapshot as O } from "./engine/completion-snapshot.js";
...
import { WasmTexPdftexEngine as M } from "./engine/wasmtex-engine.js";
import { createCompileEngine as N, unavailableEngineResult as P } from "./engine/compile-engine.js";
import { buildDependencyManifest as F, buildIncrementalDependencyManifest as I, normalizeProjectDependencyPath as L } from "./engine/dependency-manifest.js";
import { resolveEngine as R } from "./engine/engine-select.js";
import { IncrementalCompiler as z } from "./engine/incremental.js";
import { RerunController as B, signatureOf as V } from "./engine/rerun-controller.js";
import { syncAllFilesToEngine as H } from "./fs/engine-sync.js";
import { VirtualFS as U } from "./fs/virtual-fs.js";
import { parseAuxFile as W } from "./lsp/aux-parser.js";
import { parseBibFile as G, rebuildBibIndex as K } from "./lsp/bib-parser.js";
import { ProjectIndex as q } from "./lsp/project-index.js";
import { parseTraceFile as J } from "./lsp/trace-parser.js";
import { MakeindexEngine as Y } from "./engine/makeindex-engine.js";
...
var Z = class {
	...
	constructor(e = {}) {
		this.opts = e, this.mainFile = e.mainFile ?? "main.tex", this.assetBaseUrl = X(e.assetBaseUrl), this.fs = new U({ empty: !0 });
	...
	async ensureEngine() {
		if (this.detection = R(this.mainSource(), this.opts.engine), !(this.engine && this.detection.engine === this.engineKind)) {
			this.engine?.terminate(), this.engineKind = this.detection.engine, this.engine = N(this.detection.engine, this.engineBaseOpts()), this.incremental = this.opts.incremental && this.engine instanceof M ? new z(this.engine, { mainFile: this.mainFile }) : null;
			try {
	...
	async compile() {
		if (this.ensureInitialized(), this.currentAuxiliaryDependencies.clear(), await this.ensureEngine(), this.unavailable || !this.engine) {
			let e = P(this.unavailable ?? this.detection);
		...
		let t = await e.compile(), n = await this.runAuxStages(t), r = new B();
		for (; (t.success || t.pdf) && !(!r.decide(t.log, V(t.semanticTrace ?? t.log)).rerun && !n);) await this.syncModifiedFilesToEngine(), t = await e.compile(), n = await this.runAuxStages(t);
	...
	toCompileResult(e, t) {
		...
		return n.telemetry.dependencyManifest = I(this.mainFile, this.lastFullDependencyManifest), n;
	}
	setFile(e, t) {
	...
	async loadProject(e) {
		this.fs = new U({ empty: !0 }), this.projectIndex = new q(), this.generatedFiles.clear(), this.generatedDependencyObservations.clear(), this.currentAuxiliaryDependencies.clear(), this.lastFullDependencyManifest = void 0, this.incremental?.reset();
	...
	async attachCompletionSnapshot(e) {
		...
		let t = this.engine, n = this.mainFile, r = this.fs.listFiles().filter((e) => !this.generatedFiles.has(e)).flatMap((e) => {
	...
	async syncAllFilesToEngine() {
		let e = this.engine;
		!e || this.unavailable || await H(this.fs, e, (e) => this.ensureEngineDirectories(e), this.mainFile);
	}
	async syncModifiedFilesToEngine() {
		...
		for (let n of t) await e.writeFile(n.path, n.content);
		this.fs.markSynced(t), e.setMainFile(this.mainFile);
	}
	...
	async updateMetadata(e) {
		...
		if (n && this.projectIndex.updateAuxData(W(n)), e.engineCommands?.length && this.projectIndex.updateEngineCommands(e.engineCommands), e.semanticTrace && this.projectIndex.updateSemanticTrace(J(e.semanticTrace)), e.inputFiles?.length) for (let t of e.inputFiles) {
			let e = L(t);
			if (!e) continue;
	...
	runClientBiblatexLite(e, t, n) {
		let r = Object.entries(n).flatMap(([e, t]) => G(t, e)), i = m(t), a = i.includes("*") ? r.map((e) => e.key) : i;
		return p({
			entries: r,
			citedKeys: a,
			sort: d(e)
	...
	async ensureMakeindexEngine() {
		...
		let e = new Y(this.auxEngineOpts());
