...
import { COMPLETION_SNAPSHOT_MAX_ESTIMATED_BYTES as C, COMPLETION_SNAPSHOT_SCHEMA_VERSION as w, CompletionFileDigestCache as T, createCompletionSnapshot as re } from "./engine/completion-snapshot.js";
...
import { WasmTexPdftexEngine as se } from "./engine/wasmtex-engine.js";
import { createCompileEngine as ce, unavailableEngineResult as k } from "./engine/compile-engine.js";
import { resolveEngine as A } from "./engine/engine-select.js";
...
import { PREAMBLE_SNAPSHOT_JOBNAME as W, defaultFigureWorkers as le, detectAutoBlocker as ue, detectTikzExternalization as de, figureJobSource as fe, mainJobSource as pe, parseFigureList as me, parseFigureMd5 as he } from "./engine/tikz-externalization.js";
...
var ye = [
	...
	constructor(e = {}) {
		this.opts = e, this.mainFile = e.mainFile ?? "main.tex", this.assetBaseUrl = ve(e.assetBaseUrl), this.fs = new F({ empty: !0 });
	...
	async ensureEngine() {
		if (this.detection = A(this.mainSource(), this.opts.engine), !(this.engine && this.detection.engine === this.engineKind)) {
			this.engine?.terminate(), this.engineKind = this.detection.engine, this.engine = ce(this.detection.engine, this.engineBaseOpts()), this.incremental = this.opts.incremental && this.engine instanceof se ? new j(this.engine, { mainFile: this.mainFile }) : null;
			try {
	...
	async compileIdle() {
		if (this.currentAuxiliaryDependencies.clear(), await this.ensureEngine(), this.unavailable || !this.engine) {
			let e = k(this.unavailable ?? this.detection);
		...
		for (; (r.success || r.pdf) && !(!s.decide(r.log, N(r.semanticTrace ?? r.log)).rerun && !o);) await this.syncModifiedFilesToEngine(), r = await e.compile(), i.push(r.telemetry?.resolver), o = await this.runAuxStages(r);
		return a && (r.telemetry ??= { diagnostics: E(r.log) }, r.telemetry.tikzExternalization = a), this.attachTexliveDependencies(r, i), await this.updateMetadata(r), this.attachDependencyManifest(r), await this.attachCompletionSnapshot(r), this.incremental?.noteFull(this.mainSource(), this.projectTexFiles(), r.synctex), r;
	}
	...
	attachTexliveDependencies(e, t) {
		...
		for (let e of ye) n.add(`${r}.${e}`);
		let i = H(this.opts.texliveVersion ?? "2025", this.completionProfile(), t, { excludeNames: n });
		i && (this.sessionDependencies = U(this.sessionDependencies, i), e.telemetry ??= { diagnostics: E(e.log) }, e.telemetry.texliveDependencies = this.sessionDependencies);
	}
	...
	async attachCompletionSnapshot(e) {
		...
		}))), i = t.getCompletionObservation?.(), a = await re({
			engine: this.engineKind,
			root: n,
	...
	async externalizeTikzFigures(e, t, n) {
		...
		return l !== e && n.push(l.telemetry?.resolver), o.pictureErrors = _e(l, s), c && (l.log += c), Z(l, o);
	}
	...
	*projectFileEntries() {
	...
	async ensureMakeindexEngine() {
		...
		let e = new V(this.auxEngineOpts());
