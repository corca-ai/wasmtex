...
import { IncrementalCompiler as j } from "./engine/incremental.js";
import { RerunController as M, signatureOf as N } from "./engine/rerun-controller.js";
import { syncAllFilesToEngine as P } from "./fs/engine-sync.js";
import { VirtualFS as F } from "./fs/virtual-fs.js";
import { parseAuxFile as I } from "./lsp/aux-parser.js";
import { parseBibFile as L, rebuildBibIndex as R } from "./lsp/bib-parser.js";
import { ProjectIndex as z } from "./lsp/project-index.js";
import { parseTraceFile as B } from "./lsp/trace-parser.js";
import { HeapCheckpointCompiler as V } from "./engine/heap-checkpoints.js";
import { MakeindexEngine as H } from "./engine/makeindex-engine.js";
import { buildTexliveDependencySet as U, mergeTexliveDependencySets as W } from "./engine/texlive-dependencies.js";
import { PREAMBLE_SNAPSHOT_JOBNAME as G, defaultFigureWorkers as le, detectAutoBlocker as ue, detectTikzExternalization as de, figureJobSource as fe, mainJobSource as pe, parseFigureList as me, parseFigureMd5 as he } from "./engine/tikz-externalization.js";
...
async function ge(e, t = {}) {
	let n = e.mainFile ?? "main.tex", r = e.files?.[n], i = typeof r == "string" ? r : "", { injectDocumentMetadata: a, documentClassOf: o, CLASS_SUPPORT: s } = await Y(), c = a(i, t), l = o(i), u = l && s[l] || "unknown", d = X(l, u, c.injected), f = new $({
		...e,
		files: {
			...e.files,
			[n]: c.source
	...
	try {
		return await f.init(), await J(await f.compile(), c, l, u, d);
	} finally {
		f.dispose();
	}
}
async function J(e, t, n, r, i) {
	let { kernelLacksTagging: a, inspectPdfTagging: o } = await Y(), s = !a(e.log);
...
var xe = [
	...
	constructor(e = {}) {
		this.opts = e, this.mainFile = e.mainFile ?? "main.tex", this.assetBaseUrl = be(e.assetBaseUrl), this.fs = new F({ empty: !0 });
	...
	async ensureEngine() {
		if (this.detection = ce(this.mainSource(), this.opts.engine), !(this.engine && this.detection.engine === this.engineKind)) {
			this.engine?.terminate(), this.engineKind = this.detection.engine, this.engine = oe(this.detection.engine, this.engineBaseOpts()), this.incremental = this.opts.incremental && this.engine instanceof A ? new j(this.engine, { mainFile: this.mainFile }) : null, this.heap = this.opts.incremental && this.engine instanceof A ? new V(this.engine, { mainFile: this.mainFile }) : null;
			try {
	...
	async compileIdle() {
		...
		let a = r.telemetry?.tikzExternalization, o = await this.runAuxStages(r), s = new M();
		for (; (r.success || r.pdf) && !(!s.decide(r.log, N(r.semanticTrace ?? r.log)).rerun && !o);) await this.syncModifiedFilesToEngine(), r = await e.compile(this.heapArms()), i.push(r.telemetry?.resolver), o = await this.runAuxStages(r);
		return this.heap?.noteFull(this.mainSource(), this.projectTexFiles(), r), a && (r.telemetry ??= { diagnostics: T(r.log) }, r.telemetry.tikzExternalization = a), this.attachTexliveDependencies(r, i), await this.updateMetadata(r), this.attachDependencyManifest(r), await this.attachCompletionSnapshot(r), this.incremental?.noteFull(this.mainSource(), this.projectTexFiles(), r.synctex), r;
	}
	...
	async loadProject(e) {
		this.fs = new F({ empty: !0 }), this.projectIndex = new z(), this.generatedFiles.clear(), this.generatedDependencyObservations.clear(), this.currentAuxiliaryDependencies.clear(), this.lastFullDependencyManifest = void 0, this.incremental?.reset();
	...
	setMainFile(e) {
		this.projectIndex.invalidateCompletionSnapshot(), this.mainFile = e, this.currentAuxiliaryDependencies.clear(), this.lastFullDependencyManifest = void 0, this.incremental?.setMainFile(e), this.heap?.reset(), this.initialized && this.engine && !this.unavailable && this.engine.setMainFile(e);
	}
	getProjectIndex() {
	...
	attachTexliveDependencies(e, t) {
		...
		for (let e of xe) n.add(`${r}.${e}`);
		let i = U(this.opts.texliveVersion ?? "2025", this.completionProfile(), t, { excludeNames: n });
		i && (this.sessionDependencies = W(this.sessionDependencies, i), e.telemetry ??= { diagnostics: T(e.log) }, e.telemetry.texliveDependencies = this.sessionDependencies);
	}
	...
	async attachCompletionSnapshot(e) {
		...
		}))), i = t.getCompletionObservation?.(), a = await ae({
			engine: this.engineKind,
			root: n,
	...
	async syncAllFilesToEngine() {
		let e = this.engine;
		!e || this.unavailable || await P(this.fs, {
	...
	async externalizeTikzFigures(e, t, n) {
		...
		return l !== e && n.push(l.telemetry?.resolver), o.pictureErrors = ye(l, s), c && (l.log += c), Z(l, o);
	}
	async runTikzFigureJobs(e, t, n) {
		...
		let c = globalThis.navigator, l = this.opts.tikzExternalization?.workers ?? le(c?.hardwareConcurrency, c?.deviceMemory), u = await this.ensureTikzPool(l);
		u.retain(s);
	...
	*projectFileEntries() {
	...
	async exportAccessiblePdf(e = {}) {
		this.ensureInitialized();
		let t = this.mainSource(), { injectDocumentMetadata: n, documentClassOf: r, CLASS_SUPPORT: i } = await Y(), a = n(t, e), o = r(t), s = o && i[o] || "unknown", c = X(o, s, a.injected);
		return J(await this.compileForExport(a.source), a, o, s, c);
	}
	...
	updateBibIndex() {
		R(this.fs, this.projectIndex);
	}
	async updateMetadata(e) {
		if (!this.engine) return;
		let t = this.mainFile.replace(/\.tex$/, ""), n = await this.engine.readFile(`${t}.aux`);
		if (n && this.projectIndex.updateAuxData(I(n)), e.engineCommands?.length && this.projectIndex.updateEngineCommands(e.engineCommands), e.semanticTrace && this.projectIndex.updateSemanticTrace(B(e.semanticTrace)), e.inputFiles?.length) for (let t of e.inputFiles) {
			let e = k(t);
			if (!e) continue;
	...
	runClientBiblatexLite(e, t, n) {
		let r = Object.entries(n).flatMap(([e, t]) => L(t, e)), i = m(t), a = i.includes("*") ? r.map((e) => e.key) : i;
		return p({
			entries: r,
			citedKeys: a,
			sort: d(e)
	...
	async ensureBibtexEngine() {
		...
		let e = new w(this.auxEngineOpts());
	...
	async ensureMakeindexEngine() {
		...
		let e = new H(this.auxEngineOpts());
