...
import { CLASS_SUPPORT as C, documentClassOf as w, injectDocumentMetadata as T, inspectPdfTagging as E, kernelLacksTagging as D } from "./engine/accessible-export.js";
import { COMPLETION_SNAPSHOT_MAX_ESTIMATED_BYTES as re, COMPLETION_SNAPSHOT_SCHEMA_VERSION as ie, CompletionFileDigestCache as ae, createCompletionSnapshot as oe } from "./engine/completion-snapshot.js";
import { BibtexEngine as se } from "./engine/bibtex-engine.js";
import { buildDiagnostics as O, parseTexErrors as k } from "./engine/parse-errors.js";
import { buildDependencyManifest as ce, buildIncrementalDependencyManifest as le, normalizeProjectDependencyPath as A } from "./engine/dependency-manifest.js";
import { WasmTexPdftexEngine as j } from "./engine/wasmtex-engine.js";
import { createCompileEngine as M, unavailableEngineResult as N } from "./engine/compile-engine.js";
import { resolveEngine as P } from "./engine/engine-select.js";
import { IncrementalCompiler as F } from "./engine/incremental.js";
import { RerunController as I, signatureOf as L } from "./engine/rerun-controller.js";
import { syncAllFilesToEngine as R } from "./fs/engine-sync.js";
import { VirtualFS as z } from "./fs/virtual-fs.js";
import { parseAuxFile as B } from "./lsp/aux-parser.js";
import { parseBibFile as V, rebuildBibIndex as H } from "./lsp/bib-parser.js";
import { ProjectIndex as U } from "./lsp/project-index.js";
import { parseTraceFile as W } from "./lsp/trace-parser.js";
import { MakeindexEngine as ue } from "./engine/makeindex-engine.js";
import { buildTexliveDependencySet as de, mergeTexliveDependencySets as fe } from "./engine/texlive-dependencies.js";
import { PREAMBLE_SNAPSHOT_JOBNAME as G, TikzFigurePool as pe, defaultFigureWorkers as me, detectAutoBlocker as he, detectTikzExternalization as ge, figureJobSource as _e, mainJobSource as ve, parseFigureList as ye, parseFigureMd5 as be } from "./engine/tikz-externalization.js";
...
function q(e) {
	return k(e).filter((e) => !/Reference .* undefined|Citation .* undefined|There were undefined (?:references|citations)|Label\(s\) may have changed|Rerun to get/i.test(e.message));
}
async function xe(e, t = {}) {
	let n = e.mainFile ?? "main.tex", r = e.files?.[n], i = typeof r == "string" ? r : "", a = T(i, t), o = w(i), s = o && C[o] || "unknown", c = Y(o, s, a.injected), l = new $({
		...e,
		files: {
			...e.files,
			[n]: a.source
	...
	try {
		return await l.init(), await J(await l.compile(), a, o, s, c);
	} finally {
		l.dispose();
	}
}
async function J(e, t, n, r, i) {
	let a = !D(e.log);
	a || i.push("This engine's LaTeX kernel predates tagging support (TeX Live 2025); use the TeX Live 2026 profile for accessible export.");
	let o = e.pdf ? await E(e.pdf) : null;
	return o && i.push(...Se(o, a)), {
		result: e,
		declaration: {
			lang: t.lang,
			standard: t.standard,
...
function X(e, t) {
	return e.telemetry ??= { diagnostics: O(e.log) }, e.telemetry.tikzExternalization = t, e;
}
function Ce(e, t) {
...
var Q = [
	...
	constructor(e = {}) {
		this.opts = e, this.mainFile = e.mainFile ?? "main.tex", this.assetBaseUrl = we(e.assetBaseUrl), this.fs = new z({ empty: !0 });
	...
	async ensureEngine() {
		if (this.detection = P(this.mainSource(), this.opts.engine), !(this.engine && this.detection.engine === this.engineKind)) {
			this.engine?.terminate(), this.engineKind = this.detection.engine, this.engine = M(this.detection.engine, this.engineBaseOpts()), this.incremental = this.opts.incremental && this.engine instanceof j ? new F(this.engine, { mainFile: this.mainFile }) : null;
			try {
	...
	async compileIdle() {
		if (this.currentAuxiliaryDependencies.clear(), await this.ensureEngine(), this.unavailable || !this.engine) {
			let e = N(this.unavailable ?? this.detection);
		...
		let a = r.telemetry?.tikzExternalization, o = await this.runAuxStages(r), s = new I();
		for (; (r.success || r.pdf) && !(!s.decide(r.log, L(r.semanticTrace ?? r.log)).rerun && !o);) await this.syncModifiedFilesToEngine(), r = await e.compile(), i.push(r.telemetry?.resolver), o = await this.runAuxStages(r);
		return a && (r.telemetry ??= { diagnostics: O(r.log) }, r.telemetry.tikzExternalization = a), this.attachTexliveDependencies(r, i), await this.updateMetadata(r), this.attachDependencyManifest(r), await this.attachCompletionSnapshot(r), this.incremental?.noteFull(this.mainSource(), this.projectTexFiles(), r.synctex), r;
	}
	...
	toCompileResult(e, t) {
		let n = {
			...
			telemetry: { diagnostics: O(e.log) }
		};
		return n.telemetry.dependencyManifest = le(this.mainFile, this.lastFullDependencyManifest), n;
	}
	setFile(e, t) {
	...
	async loadProject(e) {
		this.fs = new z({ empty: !0 }), this.projectIndex = new U(), this.generatedFiles.clear(), this.generatedDependencyObservations.clear(), this.currentAuxiliaryDependencies.clear(), this.lastFullDependencyManifest = void 0, this.incremental?.reset();
	...
	attachTexliveDependencies(e, t) {
		...
		for (let e of Q) n.add(`${r}.${e}`);
		let i = de(this.opts.texliveVersion ?? "2025", this.completionProfile(), t, { excludeNames: n });
		i && (this.sessionDependencies = fe(this.sessionDependencies, i), e.telemetry ??= { diagnostics: O(e.log) }, e.telemetry.texliveDependencies = this.sessionDependencies);
	}
	attachDependencyManifest(e) {
		e.telemetry ??= { diagnostics: O(e.log) };
		let t = ce({
	...
	async attachCompletionSnapshot(e) {
		...
		}))), i = t.getCompletionObservation?.(), a = await oe({
			engine: this.engineKind,
			root: n,
		...
		n !== this.mainFile || t !== this.engine || this.fs.getModifiedFiles().length > 0 || (e.telemetry ??= { diagnostics: O(e.log) }, e.telemetry.completionSnapshot = a, this.projectIndex.updateCompletionSnapshot(a));
	}
	async syncAllFilesToEngine() {
		let e = this.engine;
		!e || this.unavailable || await R(this.fs, {
	...
	tikzExternalizationKind(e = this.mainSource()) {
		let t = ge(e, this.opts.tikzExternalization?.mode ?? "document");
		return t === "inject" ? this.tikzAutoDisabled ? null : (this.tikzAutoBlocker = he(e, this.projectTexFiles().values()), this.tikzAutoBlocker ? null : t) : t;
	}
	...
	async applyTikzExternalization(e, t, n) {
		return t && (e.success || e.pdf) ? this.externalizeTikzFigures(e, t, n) : (this.opts.tikzExternalization?.mode === "auto" && this.tikzAutoBlocker && (e.telemetry ??= { diagnostics: O(e.log) }, e.telemetry.tikzExternalization = {
			...Z("auto"),
	...
	async externalizeTikzFigures(e, t, n) {
		...
		return l !== e && n.push(l.telemetry?.resolver), o.pictureErrors = Ce(l, s), c && (l.log += c), X(l, o);
	}
	async runTikzFigureJobs(e, t, n) {
		...
		if (t === "inject" && s.length < 3) {
			...
			return t.blocked = "too-few-pictures", e.telemetry ??= { diagnostics: O(e.log) }, e.telemetry.tikzExternalization = t, {
				telemetry: t,
				errors: [],
				failureLog: "",
				inline: !0
		...
		let c = globalThis.navigator, l = this.opts.tikzExternalization?.workers ?? me(c?.hardwareConcurrency, c?.deviceMemory);
		this.tikzPool ??= new pe(() => this.spawnFigureCompiler(), l, this.mainFile);
		let u = this.tikzPool;
		u.retain(s);
		let d = await Promise.all(s.map((e) => r.readFile(`${e}.md5`))), f = s.map((e, t) => ({
			...
			...Z(t === "inject" ? "auto" : "document"),
			figures: s.length,
			reused: s.length - f.length,
			workers: Math.min(l, Math.max(1, f.length))
		};
		if (e.telemetry ??= { diagnostics: O(e.log) }, e.telemetry.tikzExternalization = p, f.length === 0) return {
			telemetry: p,
			errors: K(u, s),
			failureLog: ""
		};
	...
	async readTikzFigureList(e, t) {
		...
		let r = t ? [G, e] : [e, G];
		for (let e of r) {
			let t = ye(await n.readFile(`${e}.figlist`));
	...
	*projectFileEntries() {
	...
	async exportAccessiblePdf(e = {}) {
		this.ensureInitialized();
		let t = this.mainSource(), n = T(t, e), r = w(t), i = r && C[r] || "unknown", a = Y(r, i, n.injected);
		return J(await this.compileForExport(n.source), n, r, i, a);
	}
	...
	updateBibIndex() {
		H(this.fs, this.projectIndex);
	}
	async updateMetadata(e) {
		if (!this.engine) return;
		let t = this.mainFile.replace(/\.tex$/, ""), n = await this.engine.readFile(`${t}.aux`);
		if (n && this.projectIndex.updateAuxData(B(n)), e.engineCommands?.length && this.projectIndex.updateEngineCommands(e.engineCommands), e.semanticTrace && this.projectIndex.updateSemanticTrace(W(e.semanticTrace)), e.inputFiles?.length) for (let t of e.inputFiles) {
			let e = A(t);
			if (!e) continue;
	...
	runClientBiblatexLite(e, t, n) {
		let r = Object.entries(n).flatMap(([e, t]) => V(t, e)), i = m(t), a = i.includes("*") ? r.map((e) => e.key) : i;
		return p({
			entries: r,
			citedKeys: a,
			sort: d(e)
	...
	async ensureBibtexEngine() {
		...
		let e = new se(this.auxEngineOpts());
	...
	async ensureMakeindexEngine() {
		...
		let e = new ue(this.auxEngineOpts());
