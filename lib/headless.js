...
import { MemoryCacheStore as ee, backendCacheKey as v, contentKey as y, withCache as b } from "./engine/content-cache.js";
...
import { COMPLETION_SNAPSHOT_MAX_ESTIMATED_BYTES as T, COMPLETION_SNAPSHOT_SCHEMA_VERSION as E, CompletionFileDigestCache as D, createCompletionSnapshot as O } from "./engine/completion-snapshot.js";
import { BibtexEngine as k } from "./engine/bibtex-engine.js";
import { buildDiagnostics as A, parseTexErrors as j } from "./engine/parse-errors.js";
import { buildDependencyManifest as M, buildIncrementalDependencyManifest as N, normalizeProjectDependencyPath as P } from "./engine/dependency-manifest.js";
import { WasmTexPdftexEngine as F } from "./engine/wasmtex-engine.js";
import { createCompileEngine as I, unavailableEngineResult as L } from "./engine/compile-engine.js";
import { resolveEngine as R } from "./engine/engine-select.js";
import { IncrementalCompiler as z } from "./engine/incremental.js";
import { RerunController as te, signatureOf as B } from "./engine/rerun-controller.js";
import { syncAllFilesToEngine as V } from "./fs/engine-sync.js";
import { VirtualFS as H } from "./fs/virtual-fs.js";
import { parseAuxFile as U } from "./lsp/aux-parser.js";
import { parseBibFile as W, rebuildBibIndex as G } from "./lsp/bib-parser.js";
import { ProjectIndex as K } from "./lsp/project-index.js";
import { parseTraceFile as q } from "./lsp/trace-parser.js";
import { MakeindexEngine as J } from "./engine/makeindex-engine.js";
import { buildTexliveDependencySet as Y, mergeTexliveDependencySets as X } from "./engine/texlive-dependencies.js";
import { PREAMBLE_SNAPSHOT_JOBNAME as Z, TikzFigurePool as Q, defaultFigureWorkers as ne, detectTikzExternalization as re, figureJobSource as ie, mainJobSource as ae, parseFigureList as oe, parseFigureMd5 as $ } from "./engine/tikz-externalization.js";
...
var ce = [
	...
	tikzPool = null;
	constructor(e = {}) {
		this.opts = e, this.mainFile = e.mainFile ?? "main.tex", this.assetBaseUrl = se(e.assetBaseUrl), this.fs = new H({ empty: !0 });
	...
	async ensureEngine() {
		if (this.detection = R(this.mainSource(), this.opts.engine), !(this.engine && this.detection.engine === this.engineKind)) {
			this.engine?.terminate(), this.engineKind = this.detection.engine, this.engine = I(this.detection.engine, this.engineBaseOpts()), this.incremental = this.opts.incremental && this.engine instanceof F ? new z(this.engine, { mainFile: this.mainFile }) : null;
			try {
	...
	async compileIdle() {
		if (this.currentAuxiliaryDependencies.clear(), await this.ensureEngine(), this.unavailable || !this.engine) {
			let e = L(this.unavailable ?? this.detection);
			return this.attachDependencyManifest(e), e;
		}
		let e = this.engine;
		if (await this.syncModifiedFilesToEngine(), e.setPreambleSnapshot) {
			let t = !this.opts.disablePreambleSnapshot && !S(this.mainSource());
			e.setPreambleSnapshot(t);
		}
		let t = this.tikzExternalizationKind(), n = await this.tryIncrementalFastPath(t);
		if (n) return n;
		let r = await e.compile(), i = [r.telemetry?.resolver];
		t && (r.success || r.pdf) && (r = await this.externalizeTikzFigures(r, t, i));
		let a = await this.runAuxStages(r), o = new te();
		for (; (r.success || r.pdf) && !(!o.decide(r.log, B(r.semanticTrace ?? r.log)).rerun && !a);) await this.syncModifiedFilesToEngine(), r = await e.compile(), i.push(r.telemetry?.resolver), a = await this.runAuxStages(r);
		return this.attachTexliveDependencies(r, i), await this.updateMetadata(r), this.attachDependencyManifest(r), await this.attachCompletionSnapshot(r), this.incremental?.noteFull(this.mainSource(), this.projectTexFiles(), r.synctex), r;
	}
	...
	toCompileResult(e, t) {
		let n = {
			...
			telemetry: { diagnostics: A(e.log) }
		};
		return n.telemetry.dependencyManifest = N(this.mainFile, this.lastFullDependencyManifest), n;
	}
	setFile(e, t) {
	...
	async loadProject(e) {
		this.fs = new H({ empty: !0 }), this.projectIndex = new K(), this.generatedFiles.clear(), this.generatedDependencyObservations.clear(), this.currentAuxiliaryDependencies.clear(), this.lastFullDependencyManifest = void 0, this.incremental?.reset();
	...
	dispose() {
		this.engine?.terminate(), this.engine = null, this.tikzPool?.dispose(), this.tikzPool = null, this.bibtexEngine?.terminate(), this.bibtexEngine = null, this.makeindexEngine?.terminate(), this.makeindexEngine = null, this.lastFullDependencyManifest = void 0, this.initialized = !1;
	}
	dropGeneratedFile(e) {
	...
	attachTexliveDependencies(e, t) {
		...
		for (let e of ce) n.add(`${r}.${e}`);
		let i = Y(this.opts.texliveVersion ?? "2025", this.completionProfile(), t, { excludeNames: n });
		i && (this.sessionDependencies = X(this.sessionDependencies, i), e.telemetry ??= { diagnostics: A(e.log) }, e.telemetry.texliveDependencies = this.sessionDependencies);
	}
	attachDependencyManifest(e) {
		e.telemetry ??= { diagnostics: A(e.log) };
		let t = M({
	...
	async attachCompletionSnapshot(e) {
		...
		}))), i = t.getCompletionObservation?.(), a = await O({
			engine: this.engineKind,
			root: n,
		...
		n !== this.mainFile || t !== this.engine || this.fs.getModifiedFiles().length > 0 || (e.telemetry ??= { diagnostics: A(e.log) }, e.telemetry.completionSnapshot = a, this.projectIndex.updateCompletionSnapshot(a));
	}
	async syncAllFilesToEngine() {
		let e = this.engine;
		!e || this.unavailable || await V(this.fs, {
	...
	tikzExternalizationKind(e = this.mainSource()) {
		return re(e, this.opts.tikzExternalization?.mode ?? "document");
	}
	...
	async externalizeTikzFigures(e, t, n) {
		let r = this.engine;
		if (!r) return e;
		let i = !!r.setPreambleSnapshot && !this.opts.disablePreambleSnapshot;
		if (!await this.runTikzFigureJobs(e, t, i)) return e;
		let a = e.telemetry?.tikzExternalization, o = await r.compile();
		return n.push(o.telemetry?.resolver), a && (o.telemetry ??= { diagnostics: A(o.log) }, o.telemetry.tikzExternalization = a), o;
	}
	async runTikzFigureJobs(e, t, n) {
		let r = this.engine;
		if (!r) return !1;
		let i = this.mainFile.replace(/\.tex$/i, ""), a = n ? [Z, i] : [i, Z], o = null, s = [];
		for (let e of a) if (s = oe(await r.readFile(`${e}.figlist`)), s.length > 0) {
			o = e;
			break;
		}
		if (!o) return !1;
		let c = this.opts.tikzExternalization?.workers ?? ne(globalThis.navigator?.hardwareConcurrency);
		this.tikzPool ??= new Q(() => this.spawnFigureCompiler(), c, this.mainFile);
		let l = this.tikzPool;
		l.retain(s);
		let u = await Promise.all(s.map((e) => r.readFile(`${e}.md5`))), d = s.map((e, t) => ({
			name: e,
			md5: $(u[t])
		})).filter((e) => !l.isCurrent(e.name, e.md5));
		e.telemetry ??= { diagnostics: A(e.log) };
		let f = {
			figures: s.length,
			compiled: 0,
			reused: s.length - d.length,
			failed: [],
			workers: Math.min(c, Math.max(1, d.length)),
			figureTimeMs: 0
		};
		if (e.telemetry.tikzExternalization = f, d.length === 0) return !1;
		let p = this.mainSource(), m = await l.render(d, (e) => ie(p, t, o, e), () => this.projectFileEntries());
		f.compiled = m.rendered.size, f.failed = m.failures.map((e) => e.name), f.figureTimeMs = Math.round(m.elapsedMs);
		for (let t of m.failures) e.log += `\n[wasmtex] TikZ figure job '${t.name}' failed:\n${t.log.slice(-2e3)}\n`;
		let h = [];
		for (let e of m.rendered.keys()) h.push(`${e}.pdf`);
		return await this.ensureEngineDirectories(h), await Promise.all([...m.rendered].flatMap(([e, t]) => [r.writeFile(`${e}.pdf`, t.pdf), ...t.dpth === null ? [] : [r.writeFile(`${e}.dpth`, t.dpth)]])), m.rendered.size > 0;
	}
	*projectFileEntries() {
	...
	updateBibIndex() {
		G(this.fs, this.projectIndex);
	}
	async updateMetadata(e) {
		if (!this.engine) return;
		let t = this.mainFile.replace(/\.tex$/, ""), n = await this.engine.readFile(`${t}.aux`);
		if (n && this.projectIndex.updateAuxData(U(n)), e.engineCommands?.length && this.projectIndex.updateEngineCommands(e.engineCommands), e.semanticTrace && this.projectIndex.updateSemanticTrace(q(e.semanticTrace)), e.inputFiles?.length) for (let t of e.inputFiles) {
			let e = P(t);
			if (!e) continue;
	...
	runClientBiblatexLite(e, t, n) {
		let r = Object.entries(n).flatMap(([e, t]) => W(t, e)), i = m(t), a = i.includes("*") ? r.map((e) => e.key) : i;
		return p({
			entries: r,
			citedKeys: a,
			sort: d(e)
	...
	async maybeRunMakeindex(e) {
		let t = this.engine;
		if (!t || !e.success && !e.pdf || !S(this.mainSource())) return !1;
		let n = this.mainFile.replace(/\.tex$/, "");
		if (this.fs.readFile(`${n}.ind`)) return !1;
		let r = await t.readFile(`${n}.idx`);
		if (!r?.trim()) return !1;
		let i = { idx: r }, a = await C(this.opts.backends, i) ?? await this.runClientMakeindex(n, r), o = {
			stage: "index",
			projectInputs: [],
			complete: !!a
		};
	...
	async ensureBibtexEngine() {
		...
		let e = new k(this.auxEngineOpts());
	...
	async ensureMakeindexEngine() {
		...
		let e = new J(this.auxEngineOpts());
...
//#endregion
export { e as BIBER_STAGE, c as BIBLIOGRAPHY_STAGE, t as BIBTEX_STAGE, n as BackendRegistry, T as COMPLETION_SNAPSHOT_MAX_ESTIMATED_BYTES, E as COMPLETION_SNAPSHOT_SCHEMA_VERSION, r as INDEX_STAGE, ee as MemoryCacheStore, le as WasmTexCompiler, v as backendCacheKey, l as biblatexLiteBackend, y as contentKey, o as createBiberBackend, i as createJsonTextBackend, x as createMakeindexBackend, a as createRemoteBackend, w as createXindyBackend, u as detectBiblatexBackend, d as detectBiblatexSort, f as detectBibliographyMode, S as detectIndexUse, p as generateBiblatexBbl, m as parseBcfCitedKeys, s as runRemoteBiber, g as runRemoteBibliography, C as runRemoteIndex, _ as selectBiblatexBackend, b as withCache };
