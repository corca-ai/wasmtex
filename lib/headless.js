...
import { PREAMBLE_SNAPSHOT_JOBNAME as W, defaultFigureWorkers as ue, detectAutoBlocker as de, detectTikzExternalization as fe, figureJobSource as pe, mainJobSource as me, parseFigureList as he, parseFigureMd5 as ge } from "./engine/tikz-externalization.js";
...
var Ce = [
	...
	constructor(e = {}) {
		this.opts = e, this.mainFile = e.mainFile ?? "main.tex", this.assetBaseUrl = Se(e.assetBaseUrl), this.fs = new P({ empty: !0 });
	...
	engineBaseOpts() {
		let e = {
			...
			heapCheckpoints: !!this.opts.incremental && !ye(),
	...
	async compileIdle() {
		...
		let o = r.telemetry?.tikzExternalization, s = (await this.operations.observe(this.runAuxStages(r))).resume(), c = new j();
		for (; (r.success || r.pdf) && !(!c.decide(r.log, M(r.semanticTrace ?? r.log)).rerun && !s);) (await this.operations.observe(this.syncModifiedFilesToEngine())).resume(), r = (await this.operations.observe(e.compile(this.heapArms()))).resume(), a.push(r.telemetry?.resolver), Z(i, r), s = (await this.operations.observe(this.runAuxStages(r))).resume();
		return this.heap?.noteFull(this.mainSource(), this.projectTexFiles(), r), o && (r.telemetry ??= { diagnostics: T(r.log) }, r.telemetry.tikzExternalization = o), be(r, i), this.attachTexliveDependencies(r, a), (await this.operations.observe(this.updateMetadata(r))).resume(), this.attachDependencyManifest(r), (await this.operations.observe(this.attachCompletionSnapshot(r))).resume(), this.incremental?.noteFull(this.mainSource(), this.projectTexFiles(), r.synctex), r;
	}
	...
	async replaceProject(e) {
		this.fs = new P({ empty: !0 }), this.projectIndex = new R(), this.generatedFiles.clear(), this.generatedDependencyObservations.clear(), this.currentAuxiliaryDependencies.clear(), this.lastFullDependencyManifest = void 0, this.incremental?.reset(), this.heap?.reset();
	...
	async syncAllFilesToEngine() {
		let e = this.engine;
		!e || this.unavailable || (await this.operations.observe(N(this.fs, {
	...
	async externalizeTikzFigures(e, t, n) {
		...
		return l !== e && n.push(l.telemetry?.resolver), o.pictureErrors = xe(l, s), c && (l.log += c), X(l, o);
	}
	async runTikzFigureJobs(e, t, n) {
		...
		let c = globalThis.navigator, l = this.opts.tikzExternalization?.workers ?? ue(c?.hardwareConcurrency, c?.deviceMemory), u = (await this.operations.observe(this.ensureTikzPool(l))).resume();
		u.retain(s);
		...
		let _ = (await this.operations.observe(u.render(f, (e) => pe(m, t, o, e), () => [...this.projectFileEntries(), ...h]))).resume();
	...
	async readTikzFigureList(e, t) {
		...
		let r = t ? [W, e] : [e, W];
		for (let e of r) {
			let t = he((await this.operations.observe(n.readFile(`${e}.figlist`))).resume());
	...
	*projectFileEntries() {
	...
	async updateMetadata(e) {
		...
		let t = this.mainFile.replace(/\.tex$/, ""), n = (await this.operations.observe(this.engine.readFile(`${t}.aux`))).resume();
