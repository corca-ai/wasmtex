...
function $(e, t) {
  const n = t.provenance.map((i) => `${i.sourcePath}${i.line ? `:${i.line}` : ""}`).join(", ");
  return {
    name: t.name,
    kind: t.kind,
...
function M(e) {
  const t = e.provenance === "runtime-observed";
  return {
    name: e.name,
    kind: e.kind,
...
function N(e, t, n) {
  const i = e.availability?.anyOptions, a = e.availability?.deferredOptions;
  if ((!i || i.length === 0) && (!a || a.length === 0)) return !0;
  const s = /* @__PURE__ */ new Set([
    ...n.index.getClassOptions(n.document.path),
    ...n.index.getPackageOptions(t.scope.name, n.document.path)
  ]);
  return i?.some((r) => s.has(r)) ? !0 : a?.some((r) => s.has(r)) === !0 && n.index.getActiveColorNames(n.document.path).has(e.name);
}
function L(e, t) {
  const n = e.index.getLoadedPackages(e.document.path), i = /* @__PURE__ */ new Map(), a = n.has("xcolor") || t.some((r) => r.scope.id === "package/xcolor");
  if (a || n.has("color"))
    for (const r of w(a)) c(i, r);
  const s = t.flatMap(
    (r) => r.colors.filter((o) => N(o, r, e)).map((o) => $(r, o))
  ).sort((r, o) => r.priority - o.priority || r.name.localeCompare(o.name));
  for (const r of s) c(i, r);
...
function S(e, t) {
  ...
  if (e === "rgb") return u(n.map((i) => i * 255));
  if (e === "RGB") return u(n);
...
function j(e, t, n) {
  const i = e.split("!"), a = i[0].startsWith("-"), s = i[0].replace(/^-/, "").trim(), r = t.get(s);
  ...
  for (let l = 1; l < i.length; l += 2) {
    ...
    const b = i[l + 1]?.trim() || "white", f = t.get(b), g = h(f ? m(f, t, n) : null);
    if (!g) return null;
...
function m(e, t, n = /* @__PURE__ */ new Set()) {
  const i = S(e.model, e.value);
  return i || !e.alias || n.has(e.name) ? i : (n.add(e.name), j(e.alias, t, n));
}
function A(e) {
  const t = e.document.lineAt(e.position.line), n = Math.max(0, e.position.column - 1), i = /[!\s{},=[\]]/;
  let a = Math.min(n, t.length), s = a;
  for (; a > 0 && !i.test(t[a - 1]); ) a--;
  for (; s < t.length && !i.test(t[s]); ) s++;
  return t[a] === "-" && a++, {
    prefix: t.slice(a, n),
    range: {
...
function v(e, t) {
  const n = A(e), i = L(e, t);
  return [...i.values()].filter((a) => a.name.startsWith(n.prefix)).sort((a, s) => a.name.localeCompare(s.name)).map((a) => {
    const s = m(a, i);
    return {
      label: a.name,
      kind: "variable",
      insertText: a.name,
      detail: a.alias ? `Color alias · ${a.source}` : `Color · ${a.source}`,
      documentation: B(a),
      ...
      data: {
        wasmtex: {
          domain: "color",
          ...s ? { color: { css: s } } : {},
          provenance: { source: a.source, confidence: a.confidence }
        }
      }
    };
  });
