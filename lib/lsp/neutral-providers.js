...
import { formatSignature as s, getShardEnvironments as c, parseSignature as l } from "./package-db.js";
import { formatReference as u } from "./bib-parser.js";
import { registerBibCompletionResolvers as d } from "./bib-completion.js";
import { completeColors as ee } from "./color-completion.js";
import { analyzeCompletionContext as te } from "./completion-context.js";
import { CompletionResolverRegistry as ne } from "./completion-registry.js";
import { completeProjectFiles as re } from "./file-completion.js";
import { projectCommandMetadata as ie } from "./project-command-metadata.js";
import { referenceDisplayValue as f } from "./reference-value.js";
...
function m(e = {}) {
	let t = new ne(), n = e.semanticCatalog ? new oe(e.semanticCatalog, t) : void 0;
	n && p.set(t, n), d(t), t.registerResolver("command", (e, t) => {
		let r = n?.syncProject(t.index, t.cancellationToken, t.document.path, t.waitUntil);
		return {
			items: _(e.prefix, e.prefix.length, t.index, t.document.path),
			isIncomplete: r?.isIncomplete ?? !1
		};
	}), t.registerResolver("label", (e, t) => v(e.prefix, e.prefix.length, t.index, t.document.path, e.type === "argument" ? e.command : void 0)), t.registerResolver("citation", (e, t) => b(e.prefix, e.prefix.length, t.index, t.document.path)), t.registerResolver("environment", (e, t) => {
		let r = n?.syncProject(t.index, t.cancellationToken, t.document.path, t.waitUntil), i = x(e.prefix, e.prefix.length, t.index, e.type === "argument" && e.command === "begin", t.document.path);
		return C(i, e.prefix, e.prefix.length, r?.shards ?? []), {
			items: i,
			isIncomplete: r?.isIncomplete ?? !1
		};
	...
	t.registerResolver("font-family", (e, t) => {
		let n = D(e, t, "font-family"), i = r(e, t), a = Array.isArray(i) ? {
			items: i,
			isIncomplete: !1
		} : i;
		return {
	...
	}), t.registerResolver("counter", (e, t) => D(e, t, "counter")), t.registerResolver("length", (e, t) => D(e, t, "length")), t.registerResolver("glossary-key", (e, t) => D(e, t, "glossary")), t.registerResolver("acronym-key", (e, t) => D(e, t, "acronym")), t.registerResolver("key-family", (e, t) => O(e, t)), t.registerResolver("key-value", (e, r) => e.type === "argument" ? ge(e, r, n, t) : []);
	for (let e of [
		"project-tex",
		"project-bib",
		"project-image",
		...
		"project-data",
		"project-file"
	]) t.registerResolver(e, (t, n) => re(e, t.prefix, n.document.path, n.fs));
	return t;
}
function se(e, t, n) {
	p.get(e)?.syncProject(t, n);
}
var ce = m();
...
async function le(e, t, n, r, i = {}) {
	let a = /* @__PURE__ */ new Set(), o = h(e, t, n, r, {
		...i,
		waitUntil: (e) => a.add(e)
	});
...
function de(e, t) {
	return e === "macro" ? `Package macro${g(t)}` : e === "primitive" ? "TeX primitive" : "Package command";
}
function fe(e, t) {
	let n = e;
...
function v(e, t, n, r, i) {
	let a = [], o = n.getAllLabels(r), s = /* @__PURE__ */ new Map();
	for (let e of o) s.set(e.name, (s.get(e.name) ?? 0) + 1);
	for (let r of o) {
		if (s.get(r.name) !== 1) continue;
		let o = f(n, r.name, i), c = `${r.location.file}:${r.location.line}`, l = r.context, u = [
			r.name,
			l?.title,
			o,
			c
		].filter(Boolean).join(" ");
		if (!u.toLowerCase().includes(e.toLowerCase())) continue;
		let d = [
			o ? `[${o}]` : "",
			l?.kind,
			l?.title,
		...
		a.push({
			...
			detail: d,
			filterText: u,
			...l ? { documentation: y(l.source) } : {},
			data: { wasmtex: { reference: {
				definition: { ...r.location },
				...l ? { context: { ...l } } : {}
			} } },
			replaceLength: t
		});
...
function y(e) {
	let t = Math.max(2, ...Array.from(e.matchAll(/`+/g), (e) => e[0].length)), n = "`".repeat(t + 1);
	return `${n}latex\n${e}\n${n}`;
}
function b(e, t, n, r) {
...
function S(e, t, n, r, i) {
	...
	for (let o of a) {
		...
		e.push({
			...
			kind: "module",
			insertText: o,
			detail: `Package environment${g(a)}`,
			sortText: `2_${o}`,
			replaceLength: n
		});
	}
...
function D(e, t, n) {
	...
	return [.../* @__PURE__ */ new Set([...w[n] ?? [], ...r.keys()])].filter((t) => t.startsWith(e.prefix)).sort().map((t) => {
		let i = r.get(t) ?? [], a = T(i);
		return {
			label: t,
...
function I(e, t) {
	...
	return _(r, r.length, t.index, t.document.path).map((t) => ({
		...t,
...
function L(e, t, n, r) {
	...
	let i = r.map((e) => P(e.value.type)).find(Boolean);
...
function ge(e, t, n, r) {
	let i = n?.syncScopes(k(e), t.cancellationToken, t.waitUntil) ?? {
		shards: [],
		isIncomplete: !1
	}, a = A(e, i.shards), o = z(e, t);
	...
	let s = L(e, t, r, a.flatMap((t) => t.keys.filter((t) => t.name === e.key))), c = he(e, t, r, o.filter((t) => t.name === e.key));
	return {
...
function ve(e, t) {
	let n = e.lastIndexOf(".");
	return n < 0 || !_e[t].has(e.slice(n + 1).toLowerCase()) ? null : e.slice(0, n);
}
function ye(e, t, n, r) {
...
function H(e, t) {
	return (n, r) => {
		let i = ye(n.prefix, n.prefix.length, e, r.fs);
		if (!t) return i;
		let a = t.getState(e);
		...
		let o = a.shard.resources.map((t) => be(t, n.prefix, n.prefix.length, e)).filter((e) => e !== null);
		return {
			items: U([...i, ...o]),
			isIncomplete: !1
		};
...
var xe = /\\(?:begin|end)\{(\w+\*?)\}/g, Se = RegExp(`\\\\(?:${n})\\{([^}]+)\\}`, "g"), Ce = RegExp(`\\\\(?:${e})(?:\\[[^\\]]*\\])*\\{([^}]+)\\}`, "g"), we = new RegExp(t, "g");
function W(e, t, n) {
...
function Ee(e, t, n) {
	let r = e.lineAt(t.line), i = t.column - 1, a = W(r, xe, i);
	if (a) return {
		contents: De(a[1], n),
		range: G(t.line, a)
	};
...
function G(e, t) {
	return Te(e, t.index, t[0].length);
}
function De(e, t) {
	let n = o(e);
	if (n) {
...
function ke(e, t) {
	let n = a(e);
	if (n) {
		let r = [`**\\${e}**${n.detail ? ` — ${n.detail}` : ""}`], i = l(n.snippet);
...
//#endregion
export { m as createDefaultCompletionRegistry, se as preloadSemanticCatalog, h as provideCompletionResult, le as provideCompletionResultAsync, je as provideDefinition, Ee as provideHover, Me as provideReferences };
