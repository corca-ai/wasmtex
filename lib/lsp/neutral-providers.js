...
import { formatReference as l } from "./bib-parser.js";
import { registerBibCompletionResolvers as te } from "./bib-completion.js";
import { completeColors as ne } from "./color-completion.js";
import { analyzeCompletionContext as re } from "./completion-context.js";
import { CompletionResolverRegistry as ie } from "./completion-registry.js";
import { completeProjectFiles as ae } from "./file-completion.js";
import { projectCommandMetadata as oe } from "./project-command-metadata.js";
import { registerTexSemanticShard as se } from "./semantic-catalog.js";
...
var u = class {
	...
	register(e) {
		this.registeredScopes.has(e.scope.id) || (se(this.registry, e), this.registeredScopes.add(e.scope.id));
	}
}, d = /* @__PURE__ */ new WeakMap();
function f(e = {}) {
	let t = new ie(), n = e.semanticCatalog ? new u(e.semanticCatalog, t) : void 0;
	n && d.set(t, n), te(t), t.registerResolver("command", (e, t) => {
	...
	}), t.registerResolver("label", (e, t) => v(e.prefix, e.prefix.length, t.index, t.document.path)), t.registerResolver("citation", (e, t) => y(e.prefix, e.prefix.length, t.index, t.document.path)), t.registerResolver("environment", (e, t) => {
		let r = n?.syncProject(t.index, t.cancellationToken, t.document.path, t.waitUntil), i = b(e.prefix, e.prefix.length, t.index, e.type === "argument" && e.command === "begin", t.document.path);
		return S(i, e.prefix, e.prefix.length, r?.shards ?? []), {
			items: i,
			isIncomplete: r?.isIncomplete ?? !1
		};
	}), t.registerResolver("tex-class", U("tex-class", e.resourceCatalog)), t.registerResolver("tex-package", U("tex-package", e.resourceCatalog)), t.registerResolver("bib-style", U("bib-style", e.resourceCatalog)), t.registerResolver("biblatex-style", U("biblatex-style", e.resourceCatalog));
	let r = U("font-file", e.resourceCatalog);
	t.registerResolver("font-family", (e, t) => {
		let n = E(e, t, "font-family"), i = r(e, t), a = Array.isArray(i) ? {
			items: i,
			isIncomplete: !1
		} : i;
		return {
	...
	}), t.registerResolver("counter", (e, t) => E(e, t, "counter")), t.registerResolver("length", (e, t) => E(e, t, "length")), t.registerResolver("glossary-key", (e, t) => E(e, t, "glossary")), t.registerResolver("acronym-key", (e, t) => E(e, t, "acronym")), t.registerResolver("key-family", (e, t) => D(e, t)), t.registerResolver("key-value", (e, r) => e.type === "argument" ? pe(e, r, n, t) : []);
	for (let e of [
		"project-tex",
		"project-bib",
		"project-image",
		...
		"project-data",
		"project-file"
	]) t.registerResolver(e, (t, n) => ae(e, t.prefix, n.document.path, n.fs));
	return t;
}
function ce(e, t, n) {
...
function _(e, t, n, r) {
	...
	for (let [a, o] of r.getEngineCommands()) {
		...
		e.push({
			...
			insertText: r ? g(a, o.argCount) : a,
			snippet: r,
			detail: fe(o.category, o.argCount),
			sortText: `2_${a}`,
			replaceLength: n
		});
	}
...
function E(e, t, n) {
	...
	return [.../* @__PURE__ */ new Set([...C[n] ?? [], ...r.keys()])].filter((t) => t.startsWith(e.prefix)).sort().map((t) => {
		let i = r.get(t) ?? [], a = w(i);
		return {
			label: t,
...
function I(e, t, n, r) {
	...
	let i = r.map((e) => N(e.value.type)).find(Boolean);
...
function pe(e, t, n, r) {
	let i = n?.syncScopes(O(e), t.cancellationToken, t.waitUntil) ?? {
		shards: [],
		isIncomplete: !1
	}, a = k(e, i.shards), o = R(e, t);
	...
	let s = I(e, t, r, a.flatMap((t) => t.keys.filter((t) => t.name === e.key))), c = V(e, t, r, o.filter((t) => t.name === e.key));
	return {
		items: H([...c.items, ...s.items]),
...
function Ce(e, t, n) {
	let r = e.lineAt(t.line), i = t.column - 1, a = G(r, ve, i);
	if (a) return {
		contents: we(a[1], n),
		range: K(t.line, a)
	};
...
function ke(e, t, n) {
	let r = e.lineAt(t.line), i = t.column - 1, a = G(r, J, i);
	if (a) {
...
function Ae(e, t, n) {
	let r = e.lineAt(t.line), i = t.column - 1, a = G(r, /\\label\{([^}]+)\}/g, i);
...
//#endregion
export { f as createDefaultCompletionRegistry, ce as preloadSemanticCatalog, p as provideCompletionResult, ue as provideCompletionResultAsync, ke as provideDefinition, Ce as provideHover, Ae as provideReferences };
