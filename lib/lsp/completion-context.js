...
import { analyzeBibCompletionContext as a } from "./bib-completion-context.js";
import { getCommandSignature as o } from "./package-db.js";
import { maskSpansFromTokens as s } from "./latex-parser.js";
...
function m(t, n, r, i = e) {
	if (!/^[a-zA-Z@]+$/.test(n.value)) return null;
	let a = n.end, s = !1;
	t[a] === "*" && (s = !0, a++);
	let c = `${n.value}${s ? "*" : ""}`, l = r?.getCommandArguments(c) ?? o(c) ?? [], u = h(t, a, l, i);
	return u.length ? {
		command: n.value,
		starred: s,
		groups: u
...
function h(e, t, n, r) {
	...
	for (let o = 0; o < 64; o++) {
		...
		i = p(n, i, l);
		let u = i === void 0 ? c(l) : n[i], d = r(e, t, u.balancedOptional);
		if (a.push({
			delimiter: l,
			open: t,
			contentStart: t + 1,
		...
		}), i !== void 0 && i++, t = d.end, !d.closed) break;
	}
	return a;
}
function g(e, t, n, r) {
...
function D(e, i, o) {
	try {
		if (e.path.toLowerCase().endsWith(".bib")) return a(e, i);
		let c = e.getText(), f = n(c), p = r(c, f, i), m = t(c), h = s(m);
...
function O(e, t, n, r) {
	let i = `${t.value}${e[t.end] === "*" ? "*" : ""}`, a = n.getCommandArguments(i) ?? o(i);
	if (!a?.length) return [];
	let s = m(e, t, n, r);
	if (!s) return [];
	let c = [], l = 0, u = !1, d = t.end;
	for (let e of s.groups) {
		if (e.signatureIndex === void 0) break;
		if (!e.closed) {
			u = !0;
			break;
		}
		c.push([e.contentStart, e.contentEnd], [e.open, e.end]), l = e.signatureIndex + 1, d = e.end;
	}
	return !u && l > 0 && a.slice(l).every((e) => e.kind === "optional") && c.push([t.start, d]), c;
}
//#endregion
export { D as analyzeCompletionContext, O as confirmedInvocationSelectionRanges, m as parseInvocation };
