...
function p(e, t) {
	e.registerCommandScope(t.scope.id, t.commands.flatMap((e) => {
		let t = m(e);
		return e.acceptsStar && e.argumentSyntax === "xparse-v1" ? [[e.name, t], [`${e.name}*`, t]] : [[e.name, t], [`${e.name}*`, []]];
	}), t.dependencies.map((e) => `package/${e}`));
}
function m(t) {
	if (t.provenance.some((e) => /DocumentCommand$/.test(e.extractor)) && t.argumentSyntax !== "xparse-v1") return [];
	if (t.argumentSyntax === "xparse-v1") return t.args;
	let n = e(t.name);
	return n?.some((e) => e.valueKind) ? n : t.args;
}
//#endregion
