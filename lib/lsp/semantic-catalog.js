import { CatalogIdentityError as o, readCatalogText as h, validCatalogIdentity as m, sameCatalogIdentity as l } from "./catalog-transport.js";
...
function g(r, t) {
  ...
  for (const [e, i] of Object.entries(s.scopes))
    if (!d(e) || !i || !/^(?:classes|packages)\/[^/]+\.json$/.test(i.path) || i.path.includes("..") || !/^[a-f0-9]{64}$/.test(i.sha256))
...
class w {
  ...
  loadIndex() {
    return this.indexPromise ??= this.read("index.json").then((t) => g(JSON.parse(t), this.identity)).catch((t) => {
...
export {
  ...
  u as TEX_SEMANTIC_CATALOG_SCHEMA_VERSION,
