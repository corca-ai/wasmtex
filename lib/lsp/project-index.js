...
import { structuralSelectionEstimatedBytes as t } from "./structural-selection.js";
import { parseLatexFile as n } from "./latex-parser.js";
import { parseAuxFile as r } from "./aux-parser.js";
...
function s(e) {
	...
	let a = parseInt(r.slice(0, i), 10), s = parseInt(r.slice(i + 1), 10);
...
var d = class {
	...
	allLabelsCache = null;
	updateFile(e, t) {
		this.updateFileSymbols(e, n(t, e));
	}
	...
	addToIndexes(e) {
		c(this.labelDefIndex, e.labels, (e) => e.name), c(this.labelRefIndex, e.labelRefs, (e) => e.name), c(this.citationIndex, e.citations, (e) => e.key), c(this.bibItemIndex, e.bibItems, (e) => e.key), c(this.commandIndex, e.commands, (e) => e.name), c(this.commandRefIndex, e.commandUses, (e) => e.name), c(this.envDefIndex, e.environmentDefs, (e) => e.name);
	}
	removeFromIndexes(e, t) {
		l(this.labelDefIndex, t.labels, (e) => e.name, e), l(this.labelRefIndex, t.labelRefs, (e) => e.name, e), l(this.citationIndex, t.citations, (e) => e.key, e), l(this.bibItemIndex, t.bibItems, (e) => e.key, e), l(this.commandIndex, t.commands, (e) => e.name, e), l(this.commandRefIndex, t.commandUses, (e) => e.name, e), l(this.envDefIndex, t.environmentDefs, (e) => e.name, e);
	}
	updateAux(e) {
		this.auxData = r(e);
	}
	...
	rebuildBibIndexes() {
		this.bibEntries = [...this.legacyBibEntries, ...[...this.bibFiles.values()].flatMap((e) => e.entries)], this.bibStrings = [...this.bibFiles.values()].flatMap((e) => e.strings), this.bibEntryIndex = /* @__PURE__ */ new Map(), c(this.bibEntryIndex, this.bibEntries, (e) => e.key), this.activeBibFilesCache.clear();
	}
	updateAuxData(e) {
		this.auxData = e;
	}
	...
	getRootFiles(e) {
		if (!this.files.has(e)) return [];
		let { edges: t } = this.includeGraph(), n = /* @__PURE__ */ new Set(), r = [e];
		for (; r.length > 0;) {
			let e = r.pop();
			n.has(e) || (n.add(e), r.push(...[...t.get(e) ?? []].reverse()));
		}
		return [...n];
	}
	includeGraph() {
	...
	getStats() {
		let e = 0, n = 0;
		for (let [r, i] of this.files) {
			n += r.length + JSON.stringify(i).length + t(i) / 2;
			for (let t of Object.values(i)) e += t.length;
		}
		for (let [e, t] of this.bibFiles) n += e.length + JSON.stringify(t).length;
		return n += JSON.stringify(this.legacyBibEntries).length, this.completionSnapshot && (n += JSON.stringify(this.completionSnapshot).length), {
	...
	hasCompleteAuxData() {
		return this.auxData.complete !== !1;
	}
	getAuxCitations() {
	...
	resolveLabelPage(e) {
		return this.auxData.labelDetails?.get(e)?.page;
	}
	findLabelDef(e) {
	...
	parseEngineCommands(e) {
		...
		for (let r of e) {
			let e = s(r);
			a.test(e.name) || (t.set(e.name, e), n.add(e.name));
		}
		for (let [e, n] of t) {
	...
	updateCompletionSnapshot(t) {
		...
		let i = {
			...
			line: 1,
			column: 1
		}, a = /* @__PURE__ */ new Map([["counter", n.fields.counters.values.map((e) => ({
			name: e.name,
		...
		this.engineCommands = r, this.engineEnvironments = new Set(n.fields.environments.values.map((e) => e.name)), this.runtimeValues = a, this.runtimeKeys = s, this.runtimeColors = c, this.completionSnapshot = n, this.completionSnapshotStale = !1;
	}
	...
	occurrenceLocations(e, t) {
		return t === "label" ? [...f(this.labelDefIndex.get(e)), ...f(this.labelRefIndex.get(e))] : t === "citation" ? [
			...f(this.citationIndex.get(e)),
			...f(this.bibItemIndex.get(e)),
			...f(this.bibEntryIndex.get(e))
		] : f(this.commandRefIndex.get(e));
	}
};
function f(e) {
