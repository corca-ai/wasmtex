...
function v(e) {
	let t = [], n = e.content, r = 0;
	for (;;) {
		let i = n.indexOf("\\includegraphics", r);
		if (i < 0) break;
		if (r = i + 16, e.isMasked(i)) continue;
		let a = r;
		if (n[a] === "*" && a++, n[a] === "[") {
			let e = n.indexOf("]", a + 1);
			if (e >= 0 && /(?:^|,)\s*alt\s*=/.test(n.slice(a + 1, e))) continue;
		}
		t.push({
			offset: i,
			length: 16,
...
function S(e) {
	...
	let r = [], i = /\\title(?:\[[^\]]*\])?\{|pdftitle ?=/.test(t), a = /\\DocumentMetadata\{[^}]*\blang ?=|pdflang ?=/.test(t);
	return i || r.push({
		offset: n.index,
		length: 14,
