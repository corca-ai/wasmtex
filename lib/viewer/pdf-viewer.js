...
var d = class {
	...
	forwardSearch(e, t) {
		let n = this.synctexData ? this.synctexParser.forwardLookup(this.synctexData, e, t) : null;
		if (n ??= this.textMapper.forwardLookup(e, t), !n) return;
		let r = this.pagesContainer.querySelectorAll(".pdf-page-container")[n.page - 1];
		if (!r) return;
		for (let e of this.pagesContainer.querySelectorAll(".forward-search-highlight")) e.remove();
		let i = document.createElement("div");
		i.className = "forward-search-highlight", i.style.cssText = [
			"position: absolute",
			`left: ${n.x * this.renderedScale}px`,
			`top: ${n.y * this.renderedScale}px`,
			`width: ${Math.max(n.width * this.renderedScale, 200)}px`,
			`height: ${Math.max(n.height * this.renderedScale, 20)}px`,
			"background: rgba(255, 200, 0, 0.3)",
			"border: none",
			"pointer-events: none",
			"transition: opacity 0.5s"
		].join(";"), r.style.position = "relative", r.appendChild(i), r.scrollIntoView({
			behavior: "smooth",
			block: "center"
		}), setTimeout(() => {
