...
import { createDefaultCompletionRegistry as g, preloadSemanticCatalog as u, provideCompletionResult as L, provideHover as w, provideDefinition as _, provideReferences as I } from "./lsp/neutral-providers.js";
import { ProjectIndex as p } from "./lsp/project-index.js";
...
import { formatSignature as re, getCommandPackage as ne, getCommandSignature as se, getEnvironmentSignature as ae, parseSignature as le, registerShard as me } from "./lsp/package-db.js";
import { PackageShardLoader as he } from "./lsp/package-shard-loader.js";
import { HttpTexResourceCatalogProvider as ge, InMemoryTexResourceCatalogProvider as ue, TEX_RESOURCE_CATALOG_SCHEMA_VERSION as pe } from "./lsp/resource-catalog.js";
...
class H {
  ...
  constructor(e = {}) {
    this.lint = e.lint ?? !0, this.linter = new d(this.lint), this.resourceCatalog = e.resourceCatalog, this.semanticCatalog = e.semanticCatalog, this.mainFile = e.mainFile ?? "main.tex", this.completionProfile = e.completionProfile, this.completionRegistry = e.completionRegistry ?? g({
  ...
  loadProject(e) {
    this.projectRevisionEpoch++, this.fs = new h({ empty: !0 }), this.index = new p(), this.linter = new d(this.lint);
  ...
  updateFile(e, t) {
    if (this.fs.readFile(e) !== t) {
      ...
      l(e) && (this.index.updateFile(e, t), u(this.completionRegistry, this.index)), e.toLowerCase().endsWith(".bib") && this.index.updateBibFile(e, S(t, e));
    }
  }
  removeFile(e) {
  ...
  configureCompletion(e) {
    this.completionSnapshotUpdate++, this.completionProfile = e.completionProfile, this.resourceCatalog = e.resourceCatalog, this.semanticCatalog = e.semanticCatalog, this.completionRegistry = e.completionRegistry ?? g({
      ...e.resourceCatalog ? { resourceCatalog: e.resourceCatalog } : {},
      ...e.semanticCatalog ? { semanticCatalog: e.semanticCatalog } : {}
    }), this.index.clearCompletionSnapshot(), u(this.completionRegistry, this.index);
  }
  updateAux(e) {
  ...
  async updateCompletionSnapshot(e) {
    ...
    const i = ++this.completionSnapshotUpdate, o = this.projectRevisionEpoch, n = this.fs.listFiles().flatMap((c) => {
      const a = this.fs.readFile(c);
      return a === null ? [] : [{ path: c, content: typeof a == "string" ? a : Uint8Array.from(a) }];
    }), m = await C(n), r = o === this.projectRevisionEpoch && t.identity.projectRevision === m && t.identity.root === this.mainFile && this.fs.readFile(this.mainFile) !== null;
    return i !== this.completionSnapshotUpdate ? this.index.getCompletionSnapshotState() : (this.index.updateCompletionSnapshot(t), r || this.index.invalidateCompletionSnapshot(), this.index.getCompletionSnapshotState());
  }
...
`);
  ...
  getRenameEdits(e, t, i, o) {
    const n = this.index.findSymbolAt(e, t, i);
    return n ? { edits: this.index.findAllOccurrences(n.name, n.type).map((r) => ({
      file: r.filePath,
      range: {
