...
function r(e, t, n, r = () => void 0) {
	...
	for (let u of n) {
		...
		l = n.cursor;
		let d = new o(e, c(n.tokens, u.contentRange, r), a, r).parseRoot(u.contentRange);
		s.push({
			node: d,
...
function a(e, t) {
	let n = m(e.mathRoots, t, (e) => e.contentRange);
	if (n < 0) return [];
	let r = [e.mathRoots[n].node];
	for (;;) {
		let n = e.nodes[r[r.length - 1]];
		if (!n) return r;
		let i = m(n.children, t, (t) => e.nodes[t].ranges.full);
		if (i < 0) return r;
		r.push(n.children[i]);
	}
}
var o = class {
	...
	parseSpecifiedCommand(e, t, n) {
		...
		}, s = i.arguments.find((e) => e.role === "name"), c = t.behavior === "named-surface" && s ? p(this.nodes[s.node], s.range) : d(e), l = i.arguments.find((e) => e.role === "nucleus");
		return {
			node: this.addNode({
				kind: u(t),
				children: a,
	...
	parseEnvironment(e, t) {
		let n = f(this.document.content, this.input, this.cursor);
		if (!n) return {
			node: this.addNode({
				kind: "environment",
				children: [],
	...
	environmentEnd(e) {
		...
		let n = f(this.document.content, this.input, this.cursor + 1);
