...
var i = 1, a = /* @__PURE__ */ new Set([
	...
	upsert(t) {
		...
		r && (r.input.path !== t.path || r.input.language === "markdown") && this.index.removeFile(r.input.path), this.parseCount++;
		let i = e(t.content), a = n(t.content, t.path, i);
		t.language !== "markdown" && this.index.updateFileSymbols(t.path, a);
		let o = c(t, i, a);
...
function l(e, t, n) {
	let r = [...y(t, e.length), ...n ? m(e) : []], i = t.filter((e) => e.type !== "comment" && e.type !== "verb" && !C(e.start, r)), a = [], o = null;
	for (let t of i) {
		let n = u(e, t);
		n && (!o && n.kind !== "close" ? o = {
...
function y(e, t) {
	let n = [], r = [];
	for (let t of e) t.type === "command" && b(t, r, n);
...
function w(e, n) {
	...
	for (let t of n.commands) {
		let n = E(e, r, t.location, t.name.length), a = i.get(t.name);
		a ? a.push(n) : i.set(t.name, [n]);
	}
	let a = n.commands.map((t) => ({
		kind: "definition",
		name: t.name,
		source: E(e, r, t.location, t.name.length),
		definitions: i.get(t.name) ?? []
	}));
	for (let t of n.commandUses) {
		let n = E(e, r, t.location, t.name.length);
...
function T(e, n) {
	...
	return n.includes.map((t) => ({
		path: t.path,
		type: t.type,
		source: E(e, r, t.location, t.path.length)
	}));
}
function E(e, t, n, r) {
