...
var m = class extends Error {
	...
	upsert(e, n) {
		...
		H(n);
		let s = le(e, i, o, n);
		return H(n), r && r.input.language !== "markdown" && (r.input.path !== e.path || e.language === "markdown") && this.index.removeFile(r.input.path), e.language !== "markdown" && this.index.updateFileSymbols(e.path, o), this.files.set(e.fileId, {
			input: { ...e },
			baseSyntax: s,
			syntax: s
	...
	getStats() {
		...
		return {
			...
			recoveredNodes: e.reduce((e, t) => e + t.nodes.filter(W).length, 0),
			snapshotBytes: U(e),
	...
	refreshMacroDefinitions(e = /* @__PURE__ */ new Set(), t = !1) {
		let n = v(this.files.values()), r = ne(this.macroCatalog, n), a = [];
...
function ae(e, t, n, r, i) {
	...
	})), { name: o, text: s, arguments: c, mathClass: l, ...u } = e, { editable: d, ...f } = e.ranges, p = e.provenance?.source ?? {
		fileId: t.fileId,
		path: t.path,
		range: e.ranges.full
	};
...
function C(e) {
	let t = se(e), n = t.nodes[t.root], r = n?.children.length === 1 ? t.nodes[n.children[0]] : void 0;
	return r && [
		"token",
		"modifier",
		"style",
...
function O(e, t, n) {
	...
	for (let n of t) if (n.type === "comment" || n.type === "verb" || n.type === "open" || n.type === "close") r.push([n.start, n.end]);
		...
		let t = T.get(n.value);
		r.push([n.start, t === void 0 ? n.end : k(e.content, n.end, t)]);
	}
	let i = [], a = 0;
...
function N(e, t, n, r) {
	...
	return P(i, e, n, r), F(i, e, t), i;
}
function P(e, t, r, i) {
	let a = n(t.content), o = i.sections.map((e) => ({
		level: e.level,
		offset: B(a, e.location),
		name: e.title
	})).sort((e, t) => e.offset - t.offset), s = [];
	for (let n = 0; n < o.length; n++) {
		let i = o[n], a = t.content.length;
		for (let e = n + 1; e < o.length; e++) if (w[o[e].level] <= w[i.level]) {
			a = o[e].offset;
			break;
		}
		let c = 0;
		for (let e = n - 1; e >= 0; e--) if (w[o[e].level] < w[i.level]) {
			c = s[e];
			break;
		}
...
function F(e, t, n) {
	...
	for (let i of n) {
		...
		if (!n) continue;
		if (n.kind === "begin") {
			let a = r[r.length - 1]?.index ?? R(e, i.start), o = e.length;
			e.push({
				kind: "environment",
				parent: a,
				range: {
...
function z(e, t, r, i) {
	let a = d(e, t), o = new Set(a.filter((e) => e.kind === "macro" || e.kind === "operator" || e.kind === "paired-delimiter").map((e) => e.name)), s = n(e.content), c = (t) => {
		let n = B(s, t);
		return D(e, {
			startOffset: n,
...
//#endregion
export { f as LATEX_SYNTAX_SCHEMA_VERSION, m as LatexSyntaxCancelledError, h as LatexSyntaxService, s as MATH_COMMAND_SPECS, p as assertLatexSyntaxSchemaVersion, ce as createLatexSyntaxService, u as findLatexNotationPath, c as getMathCommandSpec };
