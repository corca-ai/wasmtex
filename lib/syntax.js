...
import { collectRichStructuralDeclarations as f } from "./structural-declarations.js";
import { LATEX_SYNTAX_SCHEMA_VERSION as p, assertLatexSyntaxSchemaVersion as ee } from "./syntax-contract.js";
...
function y(e, t, n) {
	...
	return {
		...e,
		definitions: r,
		...i === void 0 ? {} : { arguments: ae(e, i.arguments) },
...
function oe(e, t) {
	...
	return n.size === 0 ? e.nodes : e.nodes.map((t) => se(t, e, n));
}
function se(e, t, n) {
	...
	return ce(s, e.children) ? le(e, t, i, o, s) : e;
}
function ce(e, t) {
...
function b(e) {
	let t = de(e), n = t.nodes[t.root], r = n?.children.length === 1 ? t.nodes[n.children[0]] : void 0;
	return r && [
		"token",
		"modifier",
		"style",
...
function j(e, t) {
	let n = [];
	for (let r of t) {
		if (r.type !== "command" || !C.has(r.value)) continue;
		let t = O(e.content, r.end, w);
		n.push({
			kind: "citation",
			name: r.value,
			range: {
...
function z(e, t, n, i) {
	...
	for (let r = 0; r < o.length; r++) {
		...
		for (let e = r + 1; e < o.length; e++) if (x[o[e].level] <= x[i.level]) {
			a = o[e].offset;
			break;
		}
		let c = 0;
		for (let e = r - 1; e >= 0; e--) if (x[o[e].level] < x[i.level]) {
			c = s[e];
			break;
		}
...
function _e(e, t, n, i) {
	let a = f(e, t), o = new Set(a.filter((e) => e.kind === "macro" || e.kind === "operator" || e.kind === "paired-delimiter").map((e) => e.name)), s = r(e.content), c = (t) => {
		let r = V(s, t);
		return E(e, {
			startOffset: r,
...
//#endregion
export { p as LATEX_SYNTAX_SCHEMA_VERSION, m as LatexSyntaxCancelledError, h as LatexSyntaxService, c as MATH_COMMAND_SPECS, ee as assertLatexSyntaxSchemaVersion, fe as createLatexSyntaxService, d as findLatexNotationPath, l as getMathCommandSpec };
