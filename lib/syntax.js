...
import { CITE_CMDS as n, NEWCMD_CMDS as r } from "./lsp/latex-patterns.js";
...
var m = class extends Error {
	...
	upsert(t, n) {
		let r = {
			...t,
			language: t.language ?? (/\.(?:md|markdown)$/iu.test(t.path) ? "markdown" : "latex")
		}, i = this.files.get(t.fileId);
		U(n), this.parseCount++;
		let a = e(r.content);
		U(n);
		...
		U(n);
		let s = ge(r, a, o, n);
		return U(n), i && i.input.language !== "markdown" && (i.input.path !== r.path || r.language === "markdown") && this.index.removeFile(i.input.path), r.language !== "markdown" && this.index.updateFileSymbols(r.path, o), this.files.set(r.fileId, {
			input: r,
			baseSyntax: s,
			syntax: s
		}), this.relinkDeferred || (this.lastTransferFileIds = this.refreshMacroDefinitions(/* @__PURE__ */ new Set([r.fileId]))), this.files.get(t.fileId).syntax;
	}
	move(e, t) {
...
function we(e, n, r, i) {
	let a = t(n.content), o = i.sections.map((e) => ({
		level: e.level,
		offset: H(a, e.location),
		name: e.title
	})).sort((e, t) => e.offset - t.offset), s = [];
	for (let t = 0; t < o.length; t++) {
		let i = o[t], a = n.content.length;
		for (let e = t + 1; e < o.length; e++) if (b[o[e].level] <= b[i.level]) {
			a = o[e].offset;
			break;
		}
		let c = 0;
		for (let e = t - 1; e >= 0; e--) if (b[o[e].level] < b[i.level]) {
			c = s[e];
			break;
		}
...
function Je(e, n, r, i) {
	let a = p(e, n), o = new Set(a.filter((e) => e.kind === "macro" || e.kind === "operator" || e.kind === "paired-delimiter").map((e) => e.name)), s = t(e.content), c = (t) => {
		let n = H(s, t);
		return T(e, {
			startOffset: n,
...
function rt(e, n) {
	let r = t(e.content), i = X(e.content), o = new Map(a(e.content).map((e) => [e.inputStart, e])), s = /* @__PURE__ */ new Map();
	for (let t of n.commands) {
		let n = $(e, r, t.location, t.name.length), i = s.get(t.name);
		i ? i.push(n) : s.set(t.name, [n]);
	}
	let c = n.commands.map((t) => ({
		kind: "definition",
		name: t.name,
	...
	for (let t of n.commandUses) {
		let n = $(e, r, t.location, t.name.length);
		if (!(s.get(t.name) ?? []).some((e) => e.range.startOffset === n.range.startOffset)) {
			let e = o.get(n.range.startOffset - 1), r = Z(t.name, i);
			c.push({
				kind: "call",
				name: t.name,
				source: n,
...
var it = 4, at = RegExp(`\\\\(?:${r}|DeclareMathOperator)\\*?\\{\\\\([a-zA-Z@]+)\\}(?:\\[\\d+\\])?(?:\\[[^\\]]*\\])?\\s*\\{`, "g"), ot = /\\def\\([a-zA-Z@]+)(?:#\d)*\s*\{/g;
function X(e) {
