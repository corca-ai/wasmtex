...
var D = class extends r {
	...
	async runCompile(t) {
		...
		let o = performance.now() - r, s = i.log || "", c = i.result === "ok" && (i.status === 0 || i.status === 1), p = i.pdf ? new Uint8Array(i.pdf) : null, h = i.synctex ? new Uint8Array(i.synctex) : null, g = c && i.format ? new Uint8Array(i.format) : void 0, _ = d(s), v = !!i.preambleSnapshot, y = !!i.preambleRebuilt, b = {
			success: c,
			pdf: p,
			log: s,
			errors: _,
		...
		x && (b.phaseTimings = x), E(b, i), C(b, i), w(b, i), i.semanticTrace && (b.semanticTrace = i.semanticTrace);
		let T = u(s);
		return T.length > 0 && (m(T), b.glyphCoverage = { gaps: T }), b.telemetry = {
			diagnostics: l(s, T),
			...S(a),
			dependencies: f(s, {
...
//#endregion
export { D as WasmTexPdftexEngine, T as mergeWarmupCaches };
