...
function R(c, e) {
  ...
  for (const r of [...c.notFound, ...e.notFound]) {
    const n = `${r.format}/${r.filename}`;
    if (!i.has(n)) {
      if (t.has(n))
        if (s.has(n)) t.delete(n);
        else continue;
      i.add(n), a.push(r);
    }
  }
...
${e.log || "unknown engine error"}`);
  ...
  async compile() {
    ...
    this.status = "ready";
    const s = performance.now() - e, i = t.log || "", a = t.result === "ok" && (t.status === 0 || t.status === 1), o = t.pdf ? new Uint8Array(t.pdf) : null, h = t.synctex ? new Uint8Array(t.synctex) : null, r = a && t.format ? new Uint8Array(t.format) : void 0, n = v(i), w = !!t.preambleSnapshot, g = !!t.preambleRebuilt, l = {
      success: a,
      pdf: o,
      log: i,
      errors: n,
    ...
    t.engineCommands && (l.engineCommands = t.engineCommands), t.inputFiles && (l.inputFiles = t.inputFiles), t.semanticTrace && (l.semanticTrace = t.semanticTrace);
    const d = P(i);
    return d.length > 0 && (x(d), l.glyphCoverage = { gaps: d }), l.telemetry = {
      diagnostics: W(i, d),
      dependencies: y(i, {
        inputFiles: l.inputFiles,
        source: this.sources.get(this.mainFileName)
      })
    }, a && this.maybePersistCache(), l;
  }
  /**
