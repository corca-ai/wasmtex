...
import { compileArtifact as c, hasFatalNoOutput as l } from "./compile-output.js";
import { ResolverEvidenceCollector as u } from "./resolver-evidence.js";
import { createEngineWorker as d } from "./worker-host.js";
import { buildDiagnostics as f, parseGlyphGaps as p, parseTexErrors as m } from "./parse-errors.js";
import { buildDependencyGraph as h } from "./dependency-graph.js";
import { readResponseWithProgress as g } from "./fetch-gz.js";
import { enrichGlyphSuggestions as _ } from "./glyph-suggestions.js";
import { persistIfNeeded as v } from "./persist-watermark.js";
import { normalizeProjectDependencyPath as y } from "./dependency-manifest.js";
import { PreambleSnapshotCache as b, durablePreambleKey as x, preambleSha256 as S } from "./preamble-cache.js";
import { extractPreamble as C } from "./preamble-utils.js";
...
var A = class extends r {
	...
	constructor(e) {
		...
		super(e?.heapCheckpoints ? `${s(t, r, a)}?engine=checkpoint` : s(t, r, a), e?.texliveUrl ?? null), this.formatPath = o(t, r, a), this.skipFormatPreload = !!e?.skipFormatPreload, this.version = r, this.warmupCache = e?.warmupCache, this.preambleSnapshotEnabled = !e?.disablePreambleSnapshot, this.persistentCacheEnabled = !!e?.persistentCache && n(), this.assetBaseUrl = t, this.effectiveTexliveUrl = i(e?.texliveUrl ?? null, r), this.persistentAssetOptions = {
			version: r,
			texliveUrl: this.effectiveTexliveUrl,
			mirrorRevision: e?.resolverProfile?.mirrorRevision ?? null
		}, this.preambleMirrorRevision = e?.preambleCacheIdentity?.mirrorRevision ?? null, this.resolver = new u("pdftex", e?.resolverProfile ?? {
			id: `texlive-${r}`,
			texliveYear: r,
			mirrorRevision: this.preambleMirrorRevision
		}), e?.persistentPreambleCache && this.preambleMirrorRevision && (n() || e.preambleCacheStore) && (this.preambleCache = new b(e.preambleCacheStore ? { store: e.preambleCacheStore } : {}));
	}
	async init() {
		if (this.worker) throw Error("Engine already initialized");
		this.status = "loading", await new Promise((e, t) => {
			this.worker = d(this.enginePath), this.worker.onmessage = (n) => {
	...
	async preloadTexliveFile(e, t, n) {
		try {
			let r = await fetch(n);
			if (!r.ok) return;
			let i = await r.arrayBuffer(), a = `msg-${w++}`;
			await this.postMessageWithResponse({
				cmd: "preloadtexlive",
				format: e,
				filename: t,
	...
	async injectWarmupCache(e) {
		...
		for (let e of n) {
			...
			let n = `msg-${w++}`, r = this.postMessageWithResponse({
				cmd: "preload404",
				entries: e.entries,
				source: e.source,
				msgId: n
	...
	async fetchGzWithProgress(e) {
		try {
			let t = await fetch(e);
			return t.ok ? (await g(t, this.onProgress)).buffer : null;
		} catch {
			return null;
		}
	}
	...
	async resolveWarmupCache() {
		...
		if (this.persistentCacheEnabled) {
			this.durableCache = new t(this.persistentAssetOptions);
			try {
				let t = await this.durableCache.load();
				t && (e = e ? O(t, e) : t, this.persist.lastPersisted = 0);
			} catch {}
		}
	...
	async dumpTexliveCache() {
		...
		let e = `msg-${w++}`, t = await this.postMessageWithResponse({
			cmd: "dumpcache",
			msgId: e
		}, e), n = {
			files: t.files ?? [],
	...
	async persistTexliveCache() {
		this.durableCache && await this.durableCache.saveFrom(() => this.dumpTexliveCache());
	}
	getDownloadCount() {
		return this.persist.downloadCount;
	}
	async clearCache() {
		let e = this.durableCache ?? (n() ? new t(this.persistentAssetOptions) : null);
		await Promise.all([e?.clear(), this.preambleCache?.clear()]);
	}
	maybePersistCache() {
		this.durableCache && v(this.persist, () => this.persistTexliveCache());
	}
	async buildFormat() {
	...
	async runCompile(t) {
		...
		let o = performance.now() - r, s = i.log || "", u = l(s), d = !u && i.result === "ok" && (i.status === 0 || i.status === 1), g = c(i.pdf, u), v = c(i.synctex, u), y = d && i.format ? new Uint8Array(i.format) : void 0, b = m(s), x = !!i.preambleSnapshot, S = !!i.preambleRebuilt, C = {
			success: d,
			pdf: g,
			log: s,
			errors: b,
			...
			preambleSnapshot: x,
			preambleRebuilt: S
		}, w = this.validPhaseTimings(i.phaseTimings);
		w && (C.phaseTimings = w), k(C, i), E(C, i), D(C, i), i.semanticTrace && (C.semanticTrace = i.semanticTrace);
		let O = p(s);
		return O.length > 0 && (_(O), C.glyphCoverage = { gaps: O }), C.telemetry = {
			diagnostics: f(s, O),
			...T(a),
			dependencies: h(s, {
		...
		}, d && this.maybePersistCache(), d && S && n && i.preambleFormat && i.preambleHash && i.preambleInputFiles && this.persistPreambleSnapshot(n, i.preambleFormat, i.preambleHash, i.preambleInputFiles), C;
	}
	validPhaseTimings(e) {
...
//#endregion
export { A as WasmTexPdftexEngine, O as mergeWarmupCaches };
