...
async function l(e) {
	...
	for (let i of n.matchAll(/\/FlateDecode[^>]*>>[ \r\n]{0,3}stream\r?\n/g)) {
...
async function m(e) {
	let t = await l(e), n = /\/StructTreeRoot\b/.test(t), r = /\/Marked\s+true\b/.test(t), i = /\/Lang ?\(([^)]*)\)/.exec(t)?.[1] ?? null, a = /pdfuaid:part\s*=\s*"(\d)"|<pdfuaid:part>\s*(\d)/.exec(t), o = a ? Number(a[1] ?? a[2]) : null, s = /<dc:title>[\s\S]{0,400}?<rdf:li[^>]*>([^<]*)<\/rdf:li>/.exec(t)?.[1]?.trim() || (/\/Title ?\(([^)]*)\)/.exec(t)?.[1] ?? null);
	return {
		tagged: n && r,
		lang: i,
		uaPart: o,
