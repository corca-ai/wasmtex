...
var s = class {
	...
	record(t) {
		...
		if (!n) return;
		let r = `${n.stage}\0${n.format}\0${n.requestedName}`;
		if (!this.entries.has(r) && this.entries.size >= e) {
			this.dropped++;
			return;
		}
		this.entries.set(r, n);
	}
	finish() {
...
//#endregion
export { s as ResolverEvidenceCollector, c as mergeResolverReports };
