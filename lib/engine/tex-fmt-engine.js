...
import { CompileWorkerDriver as i } from "./wasmtex-worker.js";
import { buildDiagnostics as a, parseGlyphGaps as o, parseTexErrors as s } from "./parse-errors.js";
import { buildDependencyGraph as c } from "./dependency-graph.js";
import { readResponseWithProgress as l } from "./fetch-gz.js";
import { enrichGlyphSuggestions as u } from "./glyph-suggestions.js";
import { persistIfNeeded as d } from "./persist-watermark.js";
import { mergeWarmupCaches as f } from "./wasmtex-engine.js";
//#region src/engine/tex-fmt-engine.ts
function p(e, t) {
	let n = t.assetBaseUrl ?? "/", a = t.texliveVersion ?? "2025", o = t.texliveUrl ?? null;
	return new i(r(n, a, e), o, a);
}
function m(e, t) {
...
var h = class {
	...
	constructor(n, r, i, a, o) {
		this.tex = n, this.fmtFile = r, this.formatUrl = i, this.warmup = a, o && t() && (this.durableCache = new e({ version: o.version }));
	}
	async initTex() {
	...
	async preloadFormat() {
		...
		try {
			...
			if (!e.ok) return;
			let t = await l(e, this.onProgress);
	...
	injectWarmupAssets(e, t) {
		t.bloom && e.loadBloom(t.bloom), e.preload404(t.notFound);
		for (let n of t.files) e.preloadTexlive(n.format, n.filename, n.data.slice(0));
	}
	maybePersist() {
		if (!this.durableCache) return;
		let e = this.durableCache, t = [this.tex, ...this.extraCacheDrivers()];
		d(this.persist, async () => {
			let n = await Promise.all(t.map((e) => e.dumpCache())), r = {
				files: [],
				notFound: []
			};
			for (let e of n) r = f(r, {
				files: e.files,
				notFound: e.notFound
			});
	...
	result(e, t, n, r, i, l) {
		...
		return d.length > 0 && u(d), {
			...
			synctex: null,
			...i ? { inputFiles: i } : {},
			...typeof l == "boolean" ? { inputFilesComplete: l } : {},
			...d.length > 0 ? { glyphCoverage: { gaps: d } } : {},
			telemetry: {
				diagnostics: a(n, d),
				dependencies: c(n, {
					inputFiles: i,
...
//#endregion
export { h as BaseTexFmtEngine, p as createCompileWorker, m as unicodeFormatUrl };
