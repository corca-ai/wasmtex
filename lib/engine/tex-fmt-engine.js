...
var v = class {
	...
	warmup;
	lastWarmSets = [];
	warmupCandidates = /* @__PURE__ */ new Map();
	suppliedWarmup;
	durableCache = null;
	bloomBytes = null;
	persist = {
	...
	async fetchWarmupAssets() {
		...
		await Promise.all([...Array.from({ length: Math.min(i, t.length) }, async () => {
			for (; l < t.length;) {
				...
				let r = await a(`${e}pdftex/${n.dir}/${n.candidate ?? n.name}`);
				r && (c.push({
					format: n.format,
					filename: n.name,
					data: r
				}), n.candidate && n.candidate !== n.name && this.warmupCandidates.set(`${n.format}/${n.name}`, n.candidate));
			}
		})]);
		let u = await o;
	...
	result(e, t, n, r, i, o, d = []) {
		let p = d.map((e) => e && {
			...
			entries: e.entries.map((e) => {
				let t = this.warmupCandidates.get(`${e.format}/${e.requestedName}`);
				return t ? {
					...e,
					attempts: e.attempts.map((e) => e.source === "warmup-cache" && e.outcome === "hit" ? {
						...e,
						candidate: t
					} : e)
				} : e;
		...
		return m.length > 0 && f(m), {
			...
			...m.length > 0 ? { glyphCoverage: { gaps: m } } : {},
			telemetry: {
				diagnostics: s(n, m),
				...d.some(Boolean) ? { resolver: a(this.resolverProfile, p) } : {},
				dependencies: u(n, {
					inputFiles: i,
