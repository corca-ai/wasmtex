...
class z {
  ...
  async preloadFormat() {
    ...
    try {
      ...
      if (!e.ok) return;
      const r = await c(e, this.onProgress);
  ...
  async tryPreloadGzFormat() {
    ...
    try {
      ...
      if (!t.ok) return null;
      let e = await c(t, this.onProgress);
      if (e[0] === 31 && e[1] === 139) {
  ...
  async fetchWarmupAssets() {
    ...
    const { texliveUrl: t, preload: e, notFound: r, concurrency: i = 8 } = this.warmup, s = async (a) => {
      try {
        const n = await fetch(a);
        return n.ok ? await n.arrayBuffer() : null;
      } catch {
        return null;
      }
    }, m = s(`${t}bloom-filter.bin`), o = [];
    let h = 0;
    const f = async () => {
      for (; h < e.length; ) {
        const a = e[h++], n = await s(`${t}pdftex/${a.dir}/${a.name}`);
        n && o.push({ format: a.format, filename: a.name, data: n });
      }
    };
    await Promise.all([...Array.from({ length: Math.min(i, e.length) }, f)]);
    const u = await m;
    return u && this.durableCache && (this.bloomBytes = u), { bloom: u, files: o, notFound: r };
  }
  ...
  result(t, e, r, i, s, m) {
    ...
    return o.length > 0 && w(o), {
      ...
      synctex: null,
      ...s ? { inputFiles: s } : {},
      ...typeof m == "boolean" ? { inputFilesComplete: m } : {},
      ...o.length > 0 ? { glyphCoverage: { gaps: o } } : {},
      telemetry: {
        diagnostics: F(r, o),
