...
function T(e, t) {
	if (!e || typeof e != "object" || Array.isArray(e)) throw new F(`completion snapshot has an invalid ${t}`);
	return e;
}
function E(e, t, n) {
	let r = d(e, t);
	if (!r) throw new F(`completion snapshot has an invalid ${n}`);
	return r;
}
function D(e, t) {
	let n = T(e, `${t} field`);
	if (n.status !== "observed" && n.status !== "unsupported") throw new F(`completion snapshot has an invalid ${t} status`);
	if (!Array.isArray(n.values) || typeof n.complete != "boolean") throw new F(`completion snapshot has an invalid ${t} collection`);
...
function O(e, t, r, i) {
	...
	if (o.status === "unsupported") {
		if (o.values.length > 0) throw new F(`unsupported completion snapshot field ${t} contains values`);
...
function N(e) {
	...
	if (t.status === "unsupported") {
		if (t.values.length > 0) throw new F("unsupported completion snapshot field keyFamilies contains values");
...
function I(e) {
	let i = T(e, "root");
	if (i.version !== 1) throw new F(`unsupported completion snapshot version: ${String(i.version)}`);
	let a = T(i.identity, "identity"), o = E(a.projectRevision, 80, "project revision");
	if (!/^sha256:[a-f0-9]{64}$/.test(o)) throw new F("completion snapshot has an invalid project revision");
	if (![
		"pdflatex",
		"xelatex",
		"lualatex"
	].includes(String(a.engine))) throw new F("completion snapshot has an invalid engine");
	let s = T(a.profile, "profile");
	if (s.texliveYear !== "2025" && s.texliveYear !== "2026") throw new F("completion snapshot has an invalid TeX Live year");
...
function z(e, t) {
	let n = e ? R[e] : void 0, r = /^\d+$/.test(t ?? "") ? Number(t) : NaN;
...
//#endregion
export { t as COMPLETION_SNAPSHOT_MAX_ESTIMATED_BYTES, e as COMPLETION_SNAPSHOT_SCHEMA_VERSION, l as CompletionFileDigestCache, F as CompletionSnapshotValidationError, I as boundCompletionSnapshot, c as completionFileDigest, u as completionProjectRevision, L as createCompletionSnapshot, U as parseEngineCompletionObservation };
