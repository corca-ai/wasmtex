...
var o = class extends t {
	...
	async compile() {
		...
		let s = await this.dvipdfm.run("compilepdf"), c = `${r.log}\n${s.log}`, l = this.result(s.success && !!s.out, s.out, c, t, r.inputFiles, r.inputFilesComplete, [r.resolver, s.resolver]);
		s.inputFiles && (l.pdfConversionInputs = s.inputFiles);
		let { pages: u, placements: d, reliable: f } = a(r.out);
