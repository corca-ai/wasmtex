...
function p(e, t) {
	...
	return n.on("message", (e) => {
		d && f("recv", e?.cmd ?? e?.result), i ? i({ data: e }) : r.push(e);
	}), n.on("error", (e) => {
		f("worker error", String(e)), a?.(e);
	}), {
		postMessage(e, t) {
			d && f("send", e?.cmd), n.postMessage(e, t ?? []);
		},
		get onmessage() {
			return i;
		},
...
function g(e, t) {
	...
	try {
		let i = n(e);
		return m(t, i) && r(i).isFile() ? i : null;
	} catch {
		return null;
	}
}
function _(e) {
	if (l) throw Error("Node worker host is already installed; dispose it first");
	l = !0;
	try {
		return v(e);
	} catch (e) {
...
function v(r) {
	...
	let s = n(r.publicDir), c = (async (e, n) => {
		let r = typeof e == "string" ? e : e instanceof Request ? e.url : e.toString(), i = h(r, o, s);
		if (i.matched) {
			let e = g(i.localPath, s), n = e !== null;
			return f("fetch local", n ? "HIT" : "404", r), n ? new Response(t(e)) : new Response(null, { status: 404 });
		}
		return f("fetch cdn", r), a(e, n);
	}), u = e((e) => {
		let t = h(e, o, s), n = t.matched ? g(t.localPath, s) : null;
		if (!n) throw Error(`worker asset is outside publicDir or missing: ${e}`);
		let r = g(n.replace(/\.worker\.js$/, ".wasm"), s);
	...
	return { dispose() {
		if (!d) {
			d = !0;
			try {
				globalThis.fetch === c && (globalThis.fetch = i);
			} finally {
				u(), l = !1;
			}
		}
...
//#endregion
export { _ as installNodeWorkerHost };
