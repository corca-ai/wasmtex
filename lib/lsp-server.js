	...
	dispatch(e) {
		...
		switch (t) {
			...
			case "textDocument/selectionRange":
				if (!Array.isArray(r?.positions)) throw new n(-32602, "positions must be an array");
				return r.positions.map((e) => {
					let t = o({
						...r,
						position: e
					});
					return x(this.service.getSelectionRanges(f(t.textDocument.uri), t.position.line + 1, t.position.character + 1), t.position);
				});
			case "textDocument/linkedEditingRange": {
				let { textDocument: e, position: t } = o(r), n = this.service.getLinkedEditingRanges(f(e.uri), t.line + 1, t.character + 1);
				return n ? {
					ranges: n.ranges.map(d),
					wordPattern: n.wordPattern
				} : null;
			}
...
function g(e, t) {
	...
	return e.detail && (r.detail = e.detail), e.documentation && (r.documentation = e.documentation), e.sortText && (r.sortText = e.sortText), e.filterText && (r.filterText = e.filterText), e.data && (r.data = e.data), r;
}
function _(e) {
	return {
		contents: {
...
function b() {
	return {
		...
		renameProvider: !0,
		linkedEditingRangeProvider: !0,
		selectionRangeProvider: !0
	};
}
function x(e, t) {
	let n;
	for (let t = e.length - 1; t >= 0; t--) n = {
		range: d(e[t]),
		...n ? { parent: n } : {}
	};
	return n ?? { range: {
