...
import { createEditor as v, revealLine as u, createFileModel as y } from "./editor/setup.js";
...
import { buildDiagnostics as P, parseTexErrors as M } from "./engine/parse-errors.js";
import { RerunController as D, signatureOf as I } from "./engine/rerun-controller.js";
import { WasmTexPdftexEngine as T } from "./engine/wasmtex-engine.js";
import { syncAllFilesToEngine as j } from "./fs/engine-sync.js";
import { saveOutgoingFile as A } from "./fs/save-outgoing.js";
...
/* empty css                   */
import { setErrorMarkers as H, setDiagnosticMarkers as X } from "./ui/error-markers.js";
import { PdfViewer as z } from "./viewer/pdf-viewer.js";
const q = /* @__PURE__ */ new Set([".png", ".jpg", ".jpeg", ".gif", ".svg", ".bmp", ".webp"]), _ = 500;
function G(o) {
  const e = o.lastIndexOf(".");
  return e === -1 ? !1 : q.has(o.substring(e).toLowerCase());
}
function K(o) {
...
class Ce {
  ...
  constructor(e, i, t = {}) {
    this.opts = t, this.externalEditor = !!this.opts.editor, this.mainFile = this.opts.mainFile ?? "main.tex", this.currentFile = this.mainFile, this.assetBaseUrl = Y(this.opts.assetBaseUrl);
    const s = {
    ...
    if (this.opts.texliveUrl && (s.texliveUrl = this.opts.texliveUrl), this.engine = new T(s), this.engine.onProgress = (n) => {
   ...
   */
  unavailableEngine() {
    const e = R(this.mainSource(), this.opts.engine);
          ...
          for (const [d, b] of this.models.entries())
            if (b.uri.toString() === c) {
              if (this.onFileSelect(d), l.options?.selection) {
                const f = l.options.selection;
                this.editor.setSelection(f), this.editor.revealRangeInCenter(f);
              }
              return !0;
            }
        }
  ...
  async init() {
    ...
    try {
      await this.engine.init(), await j(
        this.fs,
        this.engine,
  ...
  revealLine(e, i) {
    i && i !== this.currentFile ? (this.openFile(i), requestAnimationFrame(() => u(this.editor, e))) : u(this.editor, e);
  }
  ...
  onFileSelect(e) {
    ...
    if (this.editor && !t) {
      ...
      A(this.fs, this.currentFile, n) && this.currentFile.endsWith(".tex") && this.projectIndex.updateFile(this.currentFile, n);
    }
  ...
  showBinaryPreview(e, i) {
    if (this.previewEl) {
      if (this.revokePreviewUrl(), this.previewEl.innerHTML = "", G(e)) {
        ...
        t.textContent = `${s.toUpperCase()} file — ${K(i.length)}`, this.previewEl.appendChild(t);
      }
      this.preview?.show();
    }
  }
  ...
  handlePostCompile(e, i = !1) {
    ...
    H(e.errors, this.models.values()), this.updateAuxIndex(), this.emitOutline(), this.runDiagnostics(), this.pendingBibtex || this.maybeRunBibtex(e), this.pendingBibtex || this.maybeRecompile(e), this.emit("compile", { result: e }), this.armPrebuild();
  }
  ...
  maybeRecompile(e) {
    ...
    const i = I(e.semanticTrace ?? e.log), t = this.rerunController.decide(e.log || "", i);
    if (t.rerun) {
  ...
  runDiagnostics() {
    ...
    X(e, this.models.values()), this.emit("diagnostics", { diagnostics: e });
  }
  downloadPdf() {
