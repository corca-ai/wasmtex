...
import { createEditor as v, revealLine as f, createFileModel as y } from "./editor/setup.js";
import { BibtexEngine as S } from "./engine/bibtex-engine.js";
...
import { normalizeProjectDependencyPath as R } from "./engine/dependency-manifest.js";
import { resolveEngine as B } from "./engine/engine-select.js";
import { IncrementalCompiler as P } from "./engine/incremental.js";
import { buildDiagnostics as M, parseTexErrors as D } from "./engine/parse-errors.js";
import { RerunController as I, signatureOf as T } from "./engine/rerun-controller.js";
import { WasmTexPdftexEngine as j } from "./engine/wasmtex-engine.js";
import { syncAllFilesToEngine as A } from "./fs/engine-sync.js";
import { saveOutgoingFile as U } from "./fs/save-outgoing.js";
import { VirtualFS as p } from "./fs/virtual-fs.js";
import { parseAuxFile as m } from "./lsp/aux-parser.js";
import { rebuildBibIndex as O } from "./lsp/bib-parser.js";
import { computeDiagnostics as L } from "./lsp/diagnostic-provider.js";
import { lintSource as V } from "./lsp/linter.js";
import { createDefaultCompletionRegistry as k } from "./lsp/neutral-providers.js";
import { ProjectIndex as $ } from "./lsp/project-index.js";
import { registerLatexProviders as W } from "./lsp/register-providers.js";
import { parseTraceFile as N } from "./lsp/trace-parser.js";
import { initPerfOverlay as H, perf as a } from "./perf/metrics.js";
import { SynctexParser as X } from "./synctex/synctex-parser.js";
/* empty css                   */
import { setErrorMarkers as z, setDiagnosticMarkers as q } from "./ui/error-markers.js";
import { PdfViewer as _ } from "./viewer/pdf-viewer.js";
const G = /* @__PURE__ */ new Set([".png", ".jpg", ".jpeg", ".gif", ".svg", ".bmp", ".webp"]), K = 500;
function Y(o) {
  const e = o.lastIndexOf(".");
  return e === -1 ? !1 : G.has(o.substring(e).toLowerCase());
}
function J(o) {
...
class Pe {
  ...
  constructor(e, i, t = {}) {
    this.opts = t, this.externalEditor = !!this.opts.editor, this.mainFile = this.opts.mainFile ?? "main.tex", this.currentFile = this.mainFile, this.assetBaseUrl = Q(this.opts.assetBaseUrl);
    const s = {
    ...
    if (this.opts.texliveUrl && (s.texliveUrl = this.opts.texliveUrl), this.engine = new j(s), this.engine.onProgress = (n) => {
    ...
    }, this.opts.incremental && (this.incremental = new P(this.engine, { mainFile: this.mainFile })), this.opts.files) {
  ...
  initViewer() {
    ...
    this.pdfViewer = new _(this.previewContainer), this.pdfViewer.setInverseSearchHandler((e) => {
  ...
  async compileActiveEngine() {
    ...
    try {
      ...
      return this.incremental?.noteFull(this.mainSource(), this.projectStringFiles(), t.synctex), t;
    } finally {
  ...
  initRuntimeServices() {
    ...
    ), this.perfOverlayDispose = H(), this.unhandledRejectionHandler = (e) => {
  ...
  async init() {
    ...
    try {
      await this.engine.init(), await A(
        this.fs,
        this.engine,
  ...
  deleteFile(e) {
    ...
    return i && (this.incremental?.reset(), this.reconcileArmed = !1, this.pendingFastMerge = null, this.cancelPrebuild(), e.endsWith(".bib") && this.updateBibIndex(), this.engine.flushCache(), this.fs.markAllModified(), this.emit("filesUpdate", { files: this.fs.listFiles() }), this.currentFile === e && this.openFile(this.mainFile), this.syncAndCompile()), i;
  }
  ...
  ensureModel(e, i) {
    ...
    if (!t) {
      t = y(i, e, !this.opts.collaboration), this.models.set(e, t);
  ...
  onFileSelect(e) {
    ...
    if (this.editor && !t) {
      ...
      U(this.fs, this.currentFile, n) && this.currentFile.endsWith(".tex") && this.projectIndex.updateFile(this.currentFile, n);
    }
  ...
  showBinaryPreview(e, i) {
    if (this.previewEl) {
      if (this.revokePreviewUrl(), this.previewEl.innerHTML = "", Y(e)) {
        ...
        t.textContent = `${s.toUpperCase()} file — ${J(i.length)}`, this.previewEl.appendChild(t);
      }
      this.preview?.show();
    }
  }
  ...
  updateEngineMetadata(e) {
    e.engineCommands?.length && this.projectIndex.updateEngineCommands(e.engineCommands), e.semanticTrace ? this.projectIndex.updateSemanticTrace(N(e.semanticTrace)) : this.projectIndex.updateSemanticTrace({ labels: /* @__PURE__ */ new Set(), refs: /* @__PURE__ */ new Set() }), e.inputFiles?.length && this.updateRecordedInputMetadata(e.inputFiles);
  }
  updateRecordedInputMetadata(e) {
    for (const i of e) {
      ...
      !s || typeof s.content != "string" || (this.projectIndex.updateFile(t, s.content), this.ensureModel(t, s.content));
    }
  }
  onCompileResult(e) {
    a.end("compile");
  ...
  handlePostCompile(e, i = !1) {
    ...
    z(e.errors, this.models.values()), this.updateAuxIndex(), this.emitOutline(), this.runDiagnostics(), this.pendingBibtex || this.maybeRunBibtex(e), this.pendingBibtex || this.maybeRecompile(e), this.emit("compile", { result: e }), this.armPrebuild();
  }
  ...
  async ensureBibtexEngine() {
    ...
    this.opts.texliveUrl && (e.texliveUrl = this.opts.texliveUrl), this.bibtexEngine = new S(e), this.bibtexEngine.onFileDownload = (i) => {
