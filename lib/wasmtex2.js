import { binaryFileBlob as w } from "./binary-file.js";
import { BinaryPreviewController as F } from "./editor/binary-preview.js";
import { installOpenCodeEditorOverride as x } from "./editor/open-code-editor-override.js";
import { createEditor as v, revealLine as f, createFileModel as S } from "./editor/setup.js";
import { BibtexEngine as y } from "./engine/bibtex-engine.js";
import { unavailableEngineResult as C } from "./engine/compile-engine.js";
import { CompileScheduler as E } from "./engine/compile-scheduler.js";
import { resolveEngine as B } from "./engine/engine-select.js";
import { IncrementalCompiler as R } from "./engine/incremental.js";
import { buildDiagnostics as P, parseTexErrors as D } from "./engine/parse-errors.js";
import { RerunController as M, signatureOf as T } from "./engine/rerun-controller.js";
import { WasmTexPdftexEngine as I } from "./engine/wasmtex-engine.js";
import { syncAllFilesToEngine as U } from "./fs/engine-sync.js";
import { saveOutgoingFile as j } from "./fs/save-outgoing.js";
import { VirtualFS as p } from "./fs/virtual-fs.js";
import { parseAuxFile as m } from "./lsp/aux-parser.js";
import { rebuildBibIndex as A } from "./lsp/bib-parser.js";
import { computeDiagnostics as O } from "./lsp/diagnostic-provider.js";
import { lintSource as L } from "./lsp/linter.js";
import { ProjectIndex as V } from "./lsp/project-index.js";
import { registerLatexProviders as k } from "./lsp/register-providers.js";
import { parseTraceFile as $ } from "./lsp/trace-parser.js";
import { initPerfOverlay as W, perf as a } from "./perf/metrics.js";
import { mergeTailSynctex as N } from "./synctex/synctex-merge.js";
import { SynctexParser as H } from "./synctex/synctex-parser.js";
/* empty css                   */
import { setErrorMarkers as X, setDiagnosticMarkers as q } from "./ui/error-markers.js";
import { PdfViewer as z } from "./viewer/pdf-viewer.js";
const _ = /* @__PURE__ */ new Set([".png", ".jpg", ".jpeg", ".gif", ".svg", ".bmp", ".webp"]), G = 500;
function K(o) {
  const e = o.lastIndexOf(".");
  return e === -1 ? !1 : _.has(o.substring(e).toLowerCase());
}
function Y(o) {
  return o < 1024 ? `${o} B` : o < 1024 * 1024 ? `${(o / 1024).toFixed(1)} KB` : `${(o / (1024 * 1024)).toFixed(1)} MB`;
}
function g(o, e) {
  const i = typeof o == "string" ? document.querySelector(o) : o;
  if (!i)
...
class Be {
  ...
  constructor(e, i, t = {}) {
    this.opts = t, this.externalEditor = !!this.opts.editor, this.mainFile = this.opts.mainFile ?? "main.tex", this.currentFile = this.mainFile, this.assetBaseUrl = J(this.opts.assetBaseUrl);
    const s = {
    ...
    if (this.opts.texliveUrl && (s.texliveUrl = this.opts.texliveUrl), this.engine = new I(s), this.engine.onProgress = (n) => {
    ...
    }, this.opts.incremental && (this.incremental = new R(this.engine, { mainFile: this.mainFile })), this.opts.files) {
      this.fs = new p({ empty: !0 });
      for (const [n, r] of Object.entries(this.opts.files))
        this.fs.writeFile(n, r);
    } else
      this.fs = new p();
    if (this.editorContainer = g(e, "editor container"), this.previewContainer = g(i, "preview container"), this.editorContainer === null)
  ...
  initViewer() {
    ...
    this.pdfViewer = new z(this.previewContainer), this.pdfViewer.setInverseSearchHandler((e) => {
   ...
   */
  unavailableEngine() {
    const e = B(this.mainSource(), this.opts.engine);
  ...
  async compileActiveEngine() {
    ...
    if (e) return C(e);
    this.prebuildInFlight && await this.prebuildInFlight;
    const i = this.incremental;
    if (i && !this.reconcileArmed) {
      const s = this.mainSource(), n = this.projectStringFiles();
      if (i.canFastServe(s, n)) {
        const r = await i.tryIncremental(s, n);
        if (r?.pdf && r.final)
          return this.reconcileArmed = !0, this.pendingFastMerge = {
            tailSynctex: r.tailSynctex ?? null,
            headPageCount: r.headPageCount ?? 0,
            tailLineOffset: r.tailLineOffset ?? 0,
            headUnchangedSinceFull: !!r.headUnchangedSinceFull
          }, this.fastCompileResult(r);
      }
    }
    this.reconcileArmed = !1;
    const t = await this.engine.compile();
    return this.incremental?.noteFull(this.mainSource(), this.projectStringFiles()), t;
  }
  ...
  async runPrebuild() {
    ...
    if (!e || this.prebuildInFlight || this.pendingBibtex || this.pendingRecompile || this.reconcileArmed || this.engine.getStatus() !== "ready") return;
  ...
  initProjectModels() {
    for (const e of this.fs.listFiles()) {
      ...
      i && typeof i.content == "string" && (this.ensureModel(e, i.content), e.endsWith(".tex") && this.projectIndex.updateFile(e, i.content));
    }
  ...
  initEditorState() {
    ...
    if (this.opts.editor)
      ...
      this.editor = v(this.editorContainer, e);
    }
    const i = this.editor._codeEditorService;
    i && (this.openCodeEditorOverride = x(
      i,
      async (t, s, n, r) => {
        const l = t, h = await r(l, s, n);
        if (!h && l.resource) {
          const c = l.resource.toString();
          for (const [d, b] of this.models.entries())
            if (b.uri.toString() === c) {
              if (this.onFileSelect(d), l.options?.selection) {
                const u = l.options.selection;
                this.editor.setSelection(u), this.editor.revealRangeInCenter(u);
              }
              return !0;
            }
        }
  ...
  initBinaryPreview() {
    this.previewEl = document.createElement("div"), this.previewEl.className = "binary-preview", this.previewEl.style.display = "none", this.editorContainer?.appendChild(this.previewEl), this.preview = new F(this.previewEl);
  }
  ...
  initRuntimeServices() {
    this.lspDisposables = k(
      this.projectIndex,
      this.fs,
      (e) => this.emit("workspaceEdit", e)
    ), this.perfOverlayDispose = W(), this.unhandledRejectionHandler = (e) => {
  ...
  async init() {
    ...
    try {
      await this.engine.init(), await U(
        this.fs,
        this.engine,
  ...
  updateModels(e, i, t) {
    for (const [s, n] of Object.entries(e))
      if (this.fs.writeFile(s, n), typeof n == "string") {
        s.endsWith(".tex") && this.projectIndex.updateFile(s, n);
  ...
  setFile(e, i) {
    ...
    if (this.fs.writeFile(e, i), e.endsWith(".tex") || this.incremental?.reset(), typeof i == "string") {
      e.endsWith(".tex") && this.projectIndex.updateFile(e, i), e.endsWith(".bib") && this.updateBibIndex();
  ...
  deleteFile(e) {
    ...
    return i && (this.incremental?.reset(), e.endsWith(".bib") && this.updateBibIndex(), this.engine.flushCache(), this.fs.markAllModified(), this.emit("filesUpdate", { files: this.fs.listFiles() }), this.currentFile === e && this.openFile(this.mainFile), this.syncAndCompile()), i;
  }
  ...
  onModelChange(e, i) {
    this.preview?.shouldSuppressModelChange(e, this.currentFile) || (a.mark("total"), a.mark("debounce"), this.bibtexDone = !1, this.bibtexRunId++, this.pendingBibtex = !1, this.pendingRecompile = !1, this.reconcileArmed = !1, this.pendingFastMerge = null, this.cancelPendingRerun(), this.cancelPrebuild(), this.rerunController.reset(), e.endsWith(".tex") || this.incremental?.reset(), this.fs.writeFile(e, i), e.endsWith(".tex") && this.projectIndex.updateFile(e, i), e.endsWith(".bib") && this.updateBibIndex(), e === this.currentFile && this.emitOutline(), this.runDiagnostics(), this.emit("filechange", { path: e, content: i }), this.syncAndCompile());
  }
  onFileSelect(e) {
    const i = this.fs.getFile(e);
    if (!i) return;
    ...
    if (this.editor && !t) {
      ...
      j(this.fs, this.currentFile, n) && this.currentFile.endsWith(".tex") && this.projectIndex.updateFile(this.currentFile, n);
    }
  ...
  showBinaryPreview(e, i) {
    if (this.previewEl) {
      if (this.revokePreviewUrl(), this.previewEl.innerHTML = "", K(e)) {
        const t = w(i), s = URL.createObjectURL(t);
        this.previewUrl = s;
        ...
        t.textContent = `${s.toUpperCase()} file — ${Y(i.length)}`, this.previewEl.appendChild(t);
      }
      this.preview?.show();
    }
  }
  ...
  updateEngineMetadata(e) {
    if (e.engineCommands?.length && this.projectIndex.updateEngineCommands(e.engineCommands), e.semanticTrace ? this.projectIndex.updateSemanticTrace($(e.semanticTrace)) : this.projectIndex.updateSemanticTrace({ labels: /* @__PURE__ */ new Set(), refs: /* @__PURE__ */ new Set() }), e.inputFiles?.length) {
      for (const i of e.inputFiles)
        if (!this.projectIndex.getFileSymbols(i)) {
          const t = this.fs.getFile(i);
          t && typeof t.content == "string" && (this.projectIndex.updateFile(i, t.content), this.ensureModel(i, t.content));
        }
    }
  }
  onCompileResult(e) {
    ...
    this.updateEngineMetadata(e), e.format && this.downloadFormat(e.format), e.success && e.pdf ? this.handleSuccessfulCompile(e, s, t, i) : (a.end("total"), console.error("[engine] compilation failed. memlog:", e.log), this.setStatus(e.errors.length > 0 ? "error" : "ready", void 0, s)), this.handlePostCompile(e, i, t);
  }
  handleSuccessfulCompile(e, i, t, s = !1) {
    s && (i = { ...i, incremental: !0 });
    const n = [];
    for (const r of this.fs.listFiles()) {
      const l = this.fs.getFile(r);
      l && typeof l.content == "string" && n.push([r, l.content]);
    }
  ...
  handlePostCompile(e, i = !1, t = 0) {
    if (X(e.errors, this.models.values()), i) {
      const s = this.pendingFastMerge;
      this.pendingFastMerge = null, this.emitOutline(), this.runDiagnostics(), this.emit("compile", { result: e }), this.applyFastSynctex(s, t);
      return;
    }
    this.updateAuxIndex(), this.emitOutline(), this.runDiagnostics(), this.pendingBibtex || this.maybeRunBibtex(e), this.pendingBibtex || this.maybeRecompile(e), this.emit("compile", { result: e }), this.armPrebuild();
  }
  ...
  async applyFastSynctex(e, i) {
    let t = !1;
    if (e?.headUnchangedSinceFull && e.tailSynctex && this.lastFullSynctexData)
      try {
        const s = await this.synctexParser.parse(e.tailSynctex);
        if (i !== this.renderSeq) return;
        const n = N({
          head: this.lastFullSynctexData,
          tail: s,
          headPageCount: e.headPageCount,
          tailLineOffset: e.tailLineOffset,
          mainFile: this.mainFile,
          tailFile: "tail.tex"
        });
        n && (this.pdfViewer?.setSynctexData(n), t = !0);
      } catch {
      }
    i === this.renderSeq && (t ? this.reconcileArmed = !1 : this.scheduler.schedule());
  }
  handleSynctex(e, i) {
    e.synctex ? (a.mark("synctex-parse"), this.synctexParser.parse(e.synctex).then((t) => {
      a.end("synctex-parse"), this.lastFullSynctexData = t, i === this.renderSeq && this.pdfViewer?.setSynctexData(t);
    }).catch((t) => {
      a.end("synctex-parse"), console.warn("SyncTeX parse failed, using text-mapper fallback:", t), this.lastFullSynctexData = null, i === this.renderSeq && this.pdfViewer?.setSynctexData(null);
    })) : (this.lastFullSynctexData = null, this.pdfViewer?.setSynctexData(null));
  }
  ...
  maybeRecompile(e) {
    ...
    const i = T(e.semanticTrace ?? e.log), t = this.rerunController.decide(e.log || "", i);
    if (t.rerun) {
  ...
  async runBibtexChain(e) {
    ...
    this.fs.writeFile(`${i}.bbl`, n), this.emit("filesUpdate", { files: this.fs.listFiles() }), this.pendingRecompile = !0;
    const r = await this.engine.compile().finally(() => {
      this.pendingRecompile = !1;
    });
    this.isCurrentBibtexRun(e) && (this.onCompileResult(r), await this.syncAndCompile());
  }
  async ensureBibtexEngine() {
    ...
    this.opts.texliveUrl && (e.texliveUrl = this.opts.texliveUrl), this.bibtexEngine = new y(e), this.bibtexEngine.onFileDownload = (i) => {
  ...
  async sendFilesToBibtex(e, i, t) {
    ...
    if (!n) return;
    const r = n[1], l = r.endsWith(".bst") ? r : `${r}.bst`, h = this.fs.readFile(l);
    h != null && e.writeFile(l, h);
  }
  updateBibIndex() {
    A(this.fs, this.projectIndex);
  }
  updateAuxIndex() {
    const e = this.mainFile.replace(/\.tex$/, "");
    this.engine.readFile(`${e}.aux`).then((i) => {
      if (!i) return;
      const t = m(i), s = t.includes.map(
        (n) => this.engine.readFile(n).then((r) => r ? m(r) : null)
      );
      Promise.all(s).then((n) => {
        for (const r of n)
          if (r) {
            for (const [l, h] of r.labels) t.labels.set(l, h);
            for (const l of r.citations) t.citations.add(l);
          }
  ...
  runDiagnostics() {
    const e = O(this.projectIndex), i = this.opts.lint ?? !0;
    if (i !== !1) {
      const t = i === !0 ? void 0 : i;
      for (const s of this.fs.listFiles()) {
        if (!s.endsWith(".tex")) continue;
        const n = this.fs.getFile(s);
        n && typeof n.content == "string" && e.push(...L(n.content, s, t));
      }
    }
    q(e, this.models.values()), this.emit("diagnostics", { diagnostics: e });
  }
  downloadPdf() {
