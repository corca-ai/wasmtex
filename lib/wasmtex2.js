...
import { parseAuxFiles as E, readAuxFiles as D } from "./lsp/aux-files.js";
import { rebuildBibIndex as O } from "./lsp/bib-parser.js";
import { computeDiagnostics as k } from "./lsp/diagnostic-provider.js";
import { createDefaultCompletionRegistry as A, preloadSemanticCatalog as j } from "./lsp/neutral-providers.js";
import { ProjectIndex as M } from "./lsp/project-index.js";
import { registerLatexProviders as N } from "./lsp/register-providers.js";
import { parseTraceFile as P } from "./lsp/trace-parser.js";
/* empty css                   */
import { setDiagnosticMarkers as F, setErrorMarkers as I } from "./ui/error-markers.js";
import { PdfViewer as L } from "./viewer/pdf-viewer.js";
...
function B(e) {
	let t = e.lastIndexOf(".");
	return t !== -1 && R.has(e.substring(t).toLowerCase());
}
function V(e) {
...
var W = class {
	...
	constructor(e, t, n = {}) {
		this.opts = n, this.externalEditor = !!this.opts.editor, this.mainFile = this.opts.mainFile ?? "main.tex", this.currentFile = this.mainFile, this.assetBaseUrl = U(this.opts.assetBaseUrl);
		let r = {
		...
		if (this.editorContainer = H(e, "editor container"), this.previewContainer = H(t, "preview container"), this.editorContainer === null) throw Error("Failed to initialize editor container.");
	...
	initViewer() {
		...
		this.pdfViewer = new L(this.previewContainer), this.pdfViewer.setInverseSearchHandler((e) => {
	...
	updateProjectIndex(e, t) {
		this.projectIndex.updateFile(e, t), this.completionRegistry && j(this.completionRegistry, this.projectIndex);
	}
	initRuntimeServices() {
		this.completionRegistry = A({
		...
		}), j(this.completionRegistry, this.projectIndex), this.lspDisposables = N(this.projectIndex, this.fs, (e) => this.emit("workspaceEdit", e), "latex", this.completionRegistry), this.perfOverlayDispose = h(), this.unhandledRejectionHandler = (e) => {
	...
	showBinaryPreview(e, t) {
		if (this.previewEl) {
			if (this.revokePreviewUrl(), this.previewEl.innerHTML = "", B(e)) {
				...
				n.className = "binary-preview-info", n.textContent = `${e.substring(e.lastIndexOf(".")).toUpperCase()} file \u2014 ${V(t.length)}`, this.previewEl.appendChild(n);
			}
			this.preview?.show();
		}
	}
	...
	updateEngineMetadata(e) {
		e.semanticTrace ? this.projectIndex.updateSemanticTrace(P(e.semanticTrace)) : this.projectIndex.updateSemanticTrace({
	...
	handlePostCompile(e, t = !1) {
		...
		I(e.errors, this.models.values()), this.updateAuxIndex(), this.emitOutline(), this.runDiagnostics(), this.pendingBibtex || this.maybeRunBibtex(e), this.pendingBibtex || this.maybeRecompile(e), this.emit("compile", { result: e }), this.armPrebuild();
	}
	applyFastSynctex(e) {
	...
	maybeRecompile(e) {
		...
		if (n.rerun) {
			this.pendingRecompile = !0, this.rerunTimer = setTimeout(() => {
	...
	async runBibtexChain(e) {
		...
		if (this.bibtexDone = !0, await this.engine.writeFile(`${t}.bbl`, i), !this.isCurrentBibtexRun(e)) return;
	...
	updateAuxIndex() {
		let e = this.mainFile, t = this.fs.listFiles().map((e) => this.fs.getFile(e));
		D(`${e.replace(/\.tex$/, "")}.aux`, (e) => this.engine.readFile(e)).then((n) => {
			this.disposed || this.mainFile !== e || t.length !== this.fs.listFiles().length || !t.every((e) => e && this.fs.getFile(e.path) === e) || (this.projectIndex.updateAuxData(E(n)), this.runDiagnostics());
		}).catch(() => {});
	}
	runDiagnostics() {
		let e = k(this.projectIndex), t = this.opts.lint ?? !0;
		if (t !== !1) {
			let n = t === !0 ? void 0 : t;
			for (let t of this.fs.listFiles()) {
		...
		F(e, this.models.values()), this.emit("diagnostics", { diagnostics: e });
	}
	downloadPdf() {
