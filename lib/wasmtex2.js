import { binaryFileBlob as F } from "./binary-file.js";
import { BinaryPreviewController as x } from "./editor/binary-preview.js";
import { installOpenCodeEditorOverride as v } from "./editor/open-code-editor-override.js";
...
import { CompileScheduler as R } from "./engine/compile-scheduler.js";
import { normalizeProjectDependencyPath as P } from "./engine/dependency-manifest.js";
import { resolveEngine as B } from "./engine/engine-select.js";
import { IncrementalCompiler as M } from "./engine/incremental.js";
import { buildDiagnostics as D, parseTexErrors as I } from "./engine/parse-errors.js";
import { RerunController as j, signatureOf as T } from "./engine/rerun-controller.js";
import { WasmTexPdftexEngine as A } from "./engine/wasmtex-engine.js";
import { syncAllFilesToEngine as U } from "./fs/engine-sync.js";
import { saveOutgoingFile as O } from "./fs/save-outgoing.js";
...
import { lintSource as k } from "./lsp/linter.js";
import { preloadSemanticCatalog as g, createDefaultCompletionRegistry as $ } from "./lsp/neutral-providers.js";
import { ProjectIndex as W } from "./lsp/project-index.js";
import { registerLatexProviders as N } from "./lsp/register-providers.js";
import { parseTraceFile as H } from "./lsp/trace-parser.js";
import { initPerfOverlay as X, perf as a } from "./perf/metrics.js";
import { SynctexParser as z } from "./synctex/synctex-parser.js";
/* empty css                   */
import { setErrorMarkers as q, setDiagnosticMarkers as _ } from "./ui/error-markers.js";
import { PdfViewer as G } from "./viewer/pdf-viewer.js";
const K = /* @__PURE__ */ new Set([".png", ".jpg", ".jpeg", ".gif", ".svg", ".bmp", ".webp"]), Y = 500;
function J(o) {
  const e = o.lastIndexOf(".");
  return e === -1 ? !1 : K.has(o.substring(e).toLowerCase());
}
function Q(o) {
...
class Me {
  ...
  constructor(e, i, t = {}) {
    this.opts = t, this.externalEditor = !!this.opts.editor, this.mainFile = this.opts.mainFile ?? "main.tex", this.currentFile = this.mainFile, this.assetBaseUrl = Z(this.opts.assetBaseUrl);
    const s = {
    ...
    }, this.opts.incremental && (this.incremental = new M(this.engine, { mainFile: this.mainFile })), this.opts.files) {
      this.fs = new p({ empty: !0 });
      for (const [n, r] of Object.entries(this.opts.files))
        this.fs.writeFile(n, r);
    } else
      this.fs = new p();
    if (this.editorContainer = b(e, "editor container"), this.previewContainer = b(i, "preview container"), this.editorContainer === null)
  ...
  initEditorState() {
    ...
    i && (this.openCodeEditorOverride = v(
      i,
      async (t, s, n, r) => {
        const l = t, h = await r(l, s, n);
        if (!h && l.resource) {
          const c = l.resource.toString();
          for (const [d, w] of this.models.entries())
            if (w.uri.toString() === c) {
  ...
  initBinaryPreview() {
    this.previewEl = document.createElement("div"), this.previewEl.className = "binary-preview", this.previewEl.style.display = "none", this.editorContainer?.appendChild(this.previewEl), this.preview = new x(this.previewEl);
  }
  ...
  updateProjectIndex(e, i) {
    this.projectIndex.updateFile(e, i), this.completionRegistry && g(this.completionRegistry, this.projectIndex);
  }
  initRuntimeServices() {
    ...
    }), g(this.completionRegistry, this.projectIndex), this.lspDisposables = N(
      this.projectIndex,
      this.fs,
      (e) => this.emit("workspaceEdit", e),
      "latex",
      this.completionRegistry
    ), this.perfOverlayDispose = X(), this.unhandledRejectionHandler = (e) => {
  ...
  onModelChange(e, i) {
    this.preview?.shouldSuppressModelChange(e, this.currentFile) || (a.mark("total"), a.mark("debounce"), this.bibtexDone = !1, this.bibtexRunId++, this.pendingBibtex = !1, this.pendingRecompile = !1, this.editEpoch++, this.reconcileArmed = !1, this.pendingFastMerge = null, this.cancelPendingRerun(), this.cancelPrebuild(), this.rerunController.reset(), e.endsWith(".tex") || this.incremental?.reset(), this.fs.writeFile(e, i), e.endsWith(".tex") && this.updateProjectIndex(e, i), e.endsWith(".bib") && this.updateBibIndex(), e === this.currentFile && this.emitOutline(), this.runDiagnostics(), this.emit("filechange", { path: e, content: i }), this.syncAndCompile());
  }
  onFileSelect(e) {
    const i = this.fs.getFile(e);
    if (!i) return;
    ...
    if (this.editor && !t) {
      ...
      O(this.fs, this.currentFile, n) && this.currentFile.endsWith(".tex") && this.updateProjectIndex(this.currentFile, n);
    }
  ...
  showBinaryPreview(e, i) {
    if (this.previewEl) {
      if (this.revokePreviewUrl(), this.previewEl.innerHTML = "", J(e)) {
        const t = F(i), s = URL.createObjectURL(t);
        this.previewUrl = s;
        ...
        t.textContent = `${s.toUpperCase()} file — ${Q(i.length)}`, this.previewEl.appendChild(t);
      }
      this.preview?.show();
    }
  }
  ...
  handlePostCompile(e, i = !1) {
    ...
    q(e.errors, this.models.values()), this.updateAuxIndex(), this.emitOutline(), this.runDiagnostics(), this.pendingBibtex || this.maybeRunBibtex(e), this.pendingBibtex || this.maybeRecompile(e), this.emit("compile", { result: e }), this.armPrebuild();
  }
  ...
  async ensureBibtexEngine() {
    ...
    this.opts.texliveUrl && (e.texliveUrl = this.opts.texliveUrl), this.bibtexEngine = new C(e), this.bibtexEngine.onFileDownload = (i) => {
